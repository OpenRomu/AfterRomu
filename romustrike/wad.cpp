...
: lumps(0), lump_count(0), file(0), _name(filename)
{
...
fill_rgb(unsigned char* pal, unsigned char* idx, int size)
{
...
static unsigned char* 
fill_rgbA(unsigned char* pal, unsigned char* idx, int size)
{
...
void 
wad_file_t::get(const char* name, rgb_image_t* r)
{
     for(int i = 0; i < lump_count; i++) {
        ...
        if(!strcmpi(lumps[i].name, name)) {
            ...
            if(fread(tmp, 1, lumps[i].disksize, file) != lumps[i].disksize);  //fixme raise error
