...
Engine::Engine()
//Engine::Engine()
:CFT_ON(false),SNIPER_ON(false),respawn_time(0.0f),time_acceleration(1.0f),offset(0),mode_op(false),is_op(false),active_input(0),mode_ecoute(false),msens(0.12f),m_tchat(0.0f),gamadelta(0.0f),maximun_arme(0),curr_tournois(0),DisplayGamma(false),TOURNOIS_ON(false),TEAM_ON(false),PASS_ON(false),m_font(0),type_download(0),est_fatigue(false),fatigue(0.0f),cours(1.0f),speed(0.0f),tps_menu(0),couvre_feu(false),mp3(NULL), m_romu(NULL),old_tick(0),old_son(0),old_tchat(0),old_tick_team(0),old_pas(0),m_overlay(true),modechat(false), menu_mode(false),menu_state(0),old_menu_state(0),
  m_xcenter(0.0f),earth_quake(0.0f),m_phrase(NULL),m_chat(NULL), m_ycenter(0.0f),m_playerfile(NULL) ,g_bWasLobbyLaunched (FALSE) ,g_dpnidLocalPlayer(0),
g_lNumberOfActivePlayers(0) ,release_tir_car(true),m_bSearchingForSessions(FALSE),bConnectSuccess(FALSE), blend(0),max_joueur(4),max_modele(1),le_rendu(0),
 g_pSoundManager(NULL),old_model(0),cle_tournois(0),round(0),tps_earthquake(0.0f),romuchat_on(true),curmp3id(0),dphdl(NULL),release_mouse(true),g_camera_suivi_old(1),g_camera_suivi(0),m_bHostPlayer (false),release_tir(0), downloading(INET_IDLE),force_crouch(false),g_pDS3DBuffer(NULL),g_pDSListener(NULL),g_pSporte(NULL),g_pSound (NULL),m_tex_impact(0),marks(0) ,lagg_ms(50),idpartie(0),killer(0),g_pGraphBuilder(NULL),config("data/config.ini"),phys(false),le_socket(0),m_texj(NULL),m_tex(NULL),parts(NULL),parts2(NULL),grenades(NULL),m_cross(NULL),m_panel(NULL),SCOPE_ETA(false),BTN_DROIT(false) ,lan_mode(false)
 ,storm_off(false),output_file(NULL),mode_storm(false),time_storm(0.0f),write_count(0)
...
//o << "mp3 new" << endl;		
	...
	m_romu = new Romu("data/env/logo1.tga");
      if(!m_romu) throw out_of_memory();
	// o << "m_chat" << endl;	
	
	m_chat = new Cchat(10.0f,120.0f,10.0f);
      if(!m_chat) throw out_of_memory();
		// o << "m_chat clear" << endl;	
	...
	if(!m_phrase) throw out_of_memory();
	...
	m_panel = new Panel("data/env/ammog.bmp","data/env/ammo.bmp","data/env/smoke2.bmp","data/env/vie.bmp","data/env/players.bmp","data/env/lazer1.bmp");
	// o << "pings" << endl;	
...
//5
	...
	sessionenum->SetText("Recherche de session*Veuillez patientez...**Entrez un mot de passe*pour les parties privees*");
...
void
Engine::ChargeUnSon(char *strFileName,int id,float min,float max)
{
    ...
    // Load the wave file into a DirectSound buffer
    HRESULT hr = g_pSoundManager->Create( &lessons[id], strFileName, DSBCAPS_CTRL3D  , DS3DALG_DEFAULT);//DS3DALG_HRTF_FULL );  
    if( FAILED( hr ) || hr == DS_NO_VIRTUALIZATION )
    {
        DXTRACE_ERR_NOMSGBOX( TEXT("Create"), hr );
        return; 
...
static void
GlQuad(float x1,float y1,float x2,float y2)
{      		
	glEnable (GL_BLEND);
...
void
Engine::display_screen(float delta )
{
...
for(int da=0;da<lesbtn.size();da++)
{
	...
	{   
		if (lesbtn[da]->visible && da!=40)	//11 c l'image de la map pas ici plus loin
			lesbtn[da]->frame(m_input.xpos,m_input.ypos,m_width,m_height,m_input.left_button,config.isdebug,delta);
	...
	lesbtn[da]->visible=(  
		...
		|| 	
		( ( (da==47) || (da==3 && m_playerfile->is_autorise) || (da==4 ) ||( da==26 && false && m_playerfile->is_autorise )|| ( is_op && ((da==38 && mode_op==false) || (da==39 && mode_op==true)) )) && menu_state==0 )
		|| 	
		( ( da==5 || da==6 || da==18 || da==19 || da==46 || da==40) && menu_state==2 )
		|| 	
		( ( da==7 || da==8 || da==9 || da==10 || da==11 || da==40 || da==12 || (da==24 && PASS_ON) || (da==23 && !PASS_ON) || (da==20 && CFT_ON) || (da==21 && TEAM_ON) || (da==22 && !TEAM_ON && !SNIPER_ON  && !CFT_ON) || (da==35 && FRIENDLY==0) || (da==36 && FRIENDLY==1) || (da==37 && FRIENDLY==2)|| (da==48 && SNIPER_ON) ) && menu_state==151)
		|| 	
		( ( da==13 || da==14) && menu_state==51)
		|| 	
		( ( da==15 ) && (menu_state==110 ))
		|| 	
		( ( da==44 ||da==45 || da==25  ) && menu_state==990)
		|| 	 
		( ( da==16 || da==17) && menu_state==64)
		|| 	
		( ( da==27 || (da==28 && curr_tournois<tournoisv.size()-1) || (da==29 &&  curr_tournois >0) || (da==30 && tournoisv.size()>0  )) && menu_state==351)
		||
		...
		{	
			if (m_playerfile)
			{
				...
				if (m_playerfile->player_id==0)	
				{
						...
						active_input=0;
						char bufff[500];
						sprintf(bufff,"Vous etes deja inscris*Saisissez votre pseudo*et votre mot de passe.*Sinon allez vous inscrire sur*%s",m_xmlsession.GServerName);
...
//	g_lNumberOfActivePlayers++;
		...
		if (menu_state==0)
		{
			...
			if (m_playerfile->is_autorise && ( m_input.keys[VK_RETURN] || lesbtn[3]->m_mouseclick))
			{
								menu_state=9; 
		...
		*/
		...
		if (menu_state==1100)
		{
			...
			if (m_input.keys[VK_RETURN] || lesbtn.at(1)->m_mouseclick==true)
			{
		...
		if ((menu_state==2001) && ((m_playerfile)))
		{
...
//				Download_Mp3 (m_xmlsession.GServerName,m_playerfile->player_mp3 );
		...
		if ((menu_state==2052 )&& (downloading==INET_IDLE))
		{
			char chemin[100];
			...
			if(!(!config.isdebug && !lan_mode && strlen(MapList.List[curmapid].wad_md5)>0 && ( strcmp(cur_md5,MapList.List[curmapid].wad_md5) || strcmp(cur_md5_bsp,MapList.List[curmapid].bsp_md5))))
			{
			if(Verify_Map(m_cur_map))
			{
				load_map(m_cur_map);
				...
				/*	if (lejoueur.size()>0)
					...
					if (m_playerfile)
						m_serverid=m_xmlsession.DevenirServer (m_playerfile,m_cur_map,( CFT_ON | (TEAM_ON<<1)),m_dwMaxPlayers);
	...
	*/
		...
		if (menu_state==2053 && (downloading==INET_IDLE))
		{
	...
	//----------------------TRANSITION--PAGE PRINCIPALE-->---LISTE SERVEURS-------------------------------------------------------
		if (menu_state==1)
		{
	...
	//----------------------LISTE SERVEUR------------------------------------------------------------
		if (menu_state==2)
		{
			...
			if (m_input.keys[VK_RETURN] || lesbtn[5]->m_mouseclick)
			{
				lesconsoles[12]->SetText("LOADING MAP ...");
...
pos_y_mouse=m_height- m_input.ypos;
			...
			{
				dessus=false;
		 		if (m_input.xpos >xd && m_input.xpos<xd*3.4 && pos_y_mouse > y1-(i+1)*h    && pos_y_mouse<  y1-(i)*h  )
				{//play
					...
					if (  m_input.left_button && ((i-1+offset)<lists.count))
					{
						...
						DXUtil_ConvertGenericStringToWide (whostname, lists.list[i+offset-1].ip ); // lists.list[i+offset-1].ip
				...
				glColor3f(1.0f, 1.0f,1.0f);
					...
					if ((i-1+offset)<lists.count)
					{
						...
						sprintf(buffer,"%s%s-%s",dess,lists.list[i-1+offset].comment ,lists.list[i-1+offset].playername);
		...
		if(menu_state==351)
		{
			...
			if(curr_tournois<tournoisv.size() && curr_tournois>=0)
			{
				...
				char buff[100];
				sprintf(buff,"%d min %d sec\0",(tournoisv.at(curr_tournois).timeout)/60,(tournoisv.at(curr_tournois).timeout)%60);
		...
		if (menu_state==352)
		{
			...
			if(m_xmlsession.GetTournoisDetail(&lesekip,m_playerfile,tournoisv.at(curr_tournois).cle_noeud,tournoisv.at(curr_tournois).cle_tournois)==S_OK)
			{
				menu_state=353;
			}
			else
	...
	//----------------------TRANSITION--LISTE SERVEURS--->-PRINCIPALE---------------------------------------------------
		...
		if (menu_state==99)
		{
			...
			if (m_input.xpos >xd && m_input.xpos<xd*3 && m_input.ypos >yd*3-160 && m_input.ypos<yd*3-130)
			{//quit
					GlQuad(xd, yd+147,xd*3, yd+167);
			...
			if(menu_state==555)
			{
				...
				if(	menu_state==556 )
				{
					...
					bool pseudo_correct=(strlen(lesinput_box[2]->text.text)>0);
						...
						bool email_correct=  (strlen(lesinput_box[4]->text.text)>10) && (strcspn(lesinput_box[4]->text.text,"@")>0);
			...
			active_input=2;						
					...
					else
					{
						...
						if (m_playerfile)
						{
							...
							m_playerfile->player_id=m_xmlsession.NewPlayer(m_playerfile->player_name,m_playerfile->playermail,m_playerfile->player_pwd,error_message);  
							if (m_playerfile->player_id!=0)
							{
								m_playerfile->save();
			...
			if (m_playerfile)
			{
				...
				if(m_romu)
				{
					if(MapList.count>0)
					{
						if (!config.isdebug)
						{
							sprintf(m_romu->Message,"************************************************MERCI AUX MAPPEURS : %s - %s****************************************************",MapList.List[curmapid].Name,MapList.List[curmapid].mappeur);
							m_romu->scroll =0.0f;
...
//				SNIPER_ON=false;
		...
		if(menu_state==2264 && downloading==INET_IDLE)
		{	//donwload map
				//Download_Map (m_cur_host,m_cur_map);
				sprintf(distant_file,"romustrike/map/%s.exe",m_cur_map);
				sprintf(local_file,"%s.exe",m_cur_map);
		...
		if ((menu_state==264) && (downloading==INET_IDLE))
		{// load e la ma
...
//		 begin_orto();
	...
	if (downloading==INET_CONNECT)
	{
				...
				//	cout << "url is : " << url<<endl;
					...
					hOpen = WinHttpOpen(L"InetURL/1.0",WINHTTP_ACCESS_TYPE_DEFAULT_PROXY,
...
// Install the status callback function.
 ...
 bResults = WinHttpReceiveResponse( hReq, NULL );
											...
											if (output_file == NULL)
											{
				//							cerr << "Open local save file error: " << GetLastError() <<endl; 
											downloading=INET_ERR;
  ...
  dwSize = 0;
      if( !WinHttpQueryDataAvailable( hReq, &dwSize ) )
	  {
        ...
        sprintf(ttt, "Error %u in WinHttpQueryDataAvailable.",
                GetLastError( ) );
		envoi_msg(ttt);
	  }
      ...
      else
      {
        ...
        if( !WinHttpReadData( hReq, (LPVOID)pszOutBuffer, 
                              dwSize, &dwDownloaded ) )
		{
            char ttt[200];
   ...
   //lockequipe();
	...
	if((m_input.keys[VK_F3]) && !(relkeys[VK_F3])) 
		...
		if (m_playerfile)
		{
			...
			if (((round==0) && (cle_tournois==0)) && ((CFT_ON==false) || (CFT_ON==TRUE && CFT_nb_gign==0 && CFT_nb_terro==0 ) || ((CFT_ON==TRUE || TEAM_ON==TRUE) && (CFT_eta<2))))
			{
...
if(menu_state!=1100 && !lan_mode && !config.isdebug &&  (m_input.keys[VK_F1]) && !(relkeys[VK_F1]) && (mp3)) 
	...
	if(menu_state==444 || menu_state==555 || menu_state==151 || menu_state==110 || menu_state==990 || modechat)
		input_text_key();
...
relkeys[190]=(m_input.keys[190]);
		...
		/*
		if((strlen(lesinput_box[active_input]->text.text)<lesinput_box[active_input]->max_len) && (m_input.keys[VK_DECIMAL]) && !(relkeys[VK_DECIMAL])) 
		{ 
...
void
Engine::handle_input(float delta)
{
   //lockequipe();
	...
	if (config.isdebug || mode_op)
	{
		...
		{ 
			...
			//	nouveau->SetPos(lejoueur[VRAI]->voiture->AxeG,lejoueur[VRAI]->voiture->Get_Global_velocity());
				nouveau->SetPos(lejoueur[VRAI]->pos+vec3_t(0.0f,0.0f,30.0f),lejoueur[VRAI]->voiture->Get_Global_velocity());
			...
			//	lejoueur[VRAI]->voiture->SetPos(lejoueur[VRAI]->voiture->AxeG);
				...
				lejoueur[VRAI]->voiture->SetPos(lejoueur[VRAI]->pos-vec3_t(0.0f,0.0f,10.0f),vec3_t(0.0f,0.0f,0.0f));
...
if((m_input.keys[VK_F7]) && !(relkeys[VK_F7])) 
	...
	lejoueur[VRAI]->tir=(!couvre_feu) && lejoueur[VRAI]->mort==false && 
						(
						(
							(m_input.keys[config.keys[6]]) 
							&& 
							(mode_op || lejoueur[VRAI]->arme.munition>0)
						) 
						);
		...
		if(!modechat)
{
			...
			{
			if(strcmp(lejoueur[VRAI]->arme.name,"grenade") && strcmp(lejoueur[VRAI]->arme.name,"c4")  && strcmp(lejoueur[VRAI]->arme.name,"fumigene")  && strcmp(lejoueur[VRAI]->arme.name,"plasma") && ((lejoueur[VRAI]->occupe ==false))) 
...
*/
	...
	else
	{
		...
		if (str3 == -1) {lejoueur[VRAI]->sens = -1;}
		...
		char ch[20];
		strcpy(ch, TEXT("jump") );
		//if( (strcmp(lejoueur[VRAI]->dep ,ch)!=0) || (strcmp(lejoueur[VRAI]->dep ,ch)==0 && lejoueur[VRAI]->modele.AniActFini==true))
    ...
    if((m_input.keys['O']) && !(relkeys['O'])) { m_overlay = !m_overlay; }
	relkeys['O']=(m_input.keys['O']);

	if((m_input.keys['F']   ) && !(relkeys['F'])) { if(config.isdebug) world.wireframe_mode = !world.wireframe_mode  ; }
	relkeys['F']=(m_input.keys['F']);
	...
	if((m_input.keys[config.keys[9]]) && !(relkeys[config.keys[9]])) {
		m_cross->position++;
		if (m_cross->position>=17) {m_cross->position=1;}
	...
	//maximun_arme=max_arme;
	if(!(lejoueur[VRAI]->tir) && (lejoueur[VRAI]->occupe ==false) && ( (m_input.is_wheeling && m_input.wheel>0) || ( m_input.keys[config.keys[10]] && !relkeys[config.keys[10]] ) ))
	{ 
		if (!SCOPE_ETA)
		{
			Change_arme(true);
	...
	if( !(lejoueur[VRAI]->tir) &&  (lejoueur[VRAI]->occupe ==false) && ( (m_input.is_wheeling && m_input.wheel<=0) || ( m_input.keys[config.keys[7]] && !relkeys[config.keys[7]] ) ))
	{ 
		if (!SCOPE_ETA)
		{
			Change_arme(false);
	...
	if(strcmp(lejoueur[VRAI]->arme.name,"grenade") && strcmp(lejoueur[VRAI]->arme.name,"c4")  && strcmp(lejoueur[VRAI]->arme.name,"fumigene")  && strcmp(lejoueur[VRAI]->arme.name,"plasma") && (m_input.middle_button && (lejoueur[VRAI]->occupe ==false))) {
		lejoueur[VRAI]->reload_arme();
...
Engine::migratehost()
{
	...
	//menu_state=1111;
	if (m_playerfile && !lan_mode)
	m_serverid=m_xmlsession.DevenirServer (m_playerfile,m_cur_map,( CFT_ON | (TEAM_ON<<1)  | (SNIPER_ON<<2) ),m_dwMaxPlayers,cle_tournois,round,cur_md5,lesinput_box[6]->text.text, strlen(lesinput_box[5]->text.text)>0,les_armes_autorisees);//
	//bConnectSuccess =true;
	m_bHostPlayer = TRUE;
	menu_state=51;
	if (m_chat)
...
void
Engine::AffichePanel(float delta)
{
   	//lockequipe();
...
//m_chat->draw(delta,m_height);
	...
	m_panel->print(m_width,m_height,lejoueur[VRAI]->arme.munition,lejoueur[VRAI]->vie,g_lNumberOfActivePlayers,lejoueur[VRAI]->arme.name);
...
//parts->SystemNew(new   pSystemBoom_t,vec3_t(0.0f,0.0f,0.0f), 1); 
	...
	if(mode_storm)
	{
		...
		if(time_storm>=1.0f)
		{
			...
			storm_off=!((time_storm<eclaire[0]) || (time_storm>eclaire[1] && time_storm<eclaire[2]) || (time_storm>eclaire[3] && time_storm<eclaire[4]) );
							...
							{ 
								...
								if(TOURNOIS_ON==1 && (remain<0))
								{
									...
									if (g_lNumberOfActivePlayers>1)
									{
										...
										// a faire : quand un joueur entre dans la partie avant de faire 'LANCER' sur le menu , le rendu est tous pourri bare noir , surment du au envoi position qui envoie de la merde car pas init ??
		...
		//if (exterieur)
			...
			g_pDSListener->SetOrientation( -yyy[0],  -yyy[1], 0, top.x,  top.y,  top.z,DS3D_IMMEDIATE);
				...
				//	world.radius=10.0f;
					...
					//	yeux =lejoueur[VRAI]->m_PhysEnv.AxeG-lejoueur[VRAI]->m_PhysEnv.AxeDevant*10.0f;//lejoueur[VRAI]->m_PhysEnv.AxeHaut +
				...
				/*	if (lejoueur[VRAI]->is_car)
					{
						...
						flash =lejoueur[VRAI]->voiture->AxeG+lejoueur[VRAI]->voiture->AxeDevant*10.0f-lejoueur[VRAI]->voiture->AxeHaut*15.0f;
					}				
					*/
			...
			{ //vue voiture
				if(exterieur && world_is_loaded)
				{
					...
					m_camera.cam.load_identity ();
					m_camera.load ();
					gluLookAt(dst_cam[0] ,dst_cam[1] ,dst_cam[2] ,vue_cam[0],vue_cam[1],vue_cam[2],0,0,1); //orient ,pos,norm
					glGetFloatv(GL_MODELVIEW_MATRIX,m_camera.cam.m);
				...
				else
				{
					...
					dstf=lejoueur[VRAI]->voiture->AxeG+lejoueur[VRAI]->voiture->AxeDevant*0.2f+lejoueur[VRAI]->voiture->AxeHaut*0.15f;
					...
					m_camera.cam.load_identity ();
					m_camera.load ();
					gluLookAt(dst_cam[0] ,dst_cam[1] ,dst_cam[2] ,vue_cam[0],vue_cam[1],vue_cam[2],vue_h[0],vue_h[1],vue_h[2]); //orient ,pos,norm
					glGetFloatv(GL_MODELVIEW_MATRIX,m_camera.cam.m);
					m_camera.move(dst_cam[0],dst_cam[1],dst_cam[2]);
					yeux=m_camera.eye ();
					lejoueur[VRAI]->fps=letick;
...
//vue_cam[1]=0.0f;
		...
		//	 		gluLookAt(dst_cam[0] ,dst_cam[1] ,dst_cam[2] ,dstf[0]-vue_cam[0],dstf[1]-vue_cam[1],dstf[2]-vue_cam[2],0,0,1); //orient ,pos,norm
			gluLookAt(dst_cam[0] ,dst_cam[1] ,dst_cam[2] ,vue_cam[0],vue_cam[1],vue_cam[2],0,0,1); //orient ,pos,norm
				...
				for(int o2=0;o2<lescar.size();o2++)
				{
					if(o1!=o2)
					{
						...
						if(lin<50.0f)
						{
							...
							//vec3_t cc=(lescar[o1]->Get_Global_velocity()-lescar[o2]->Get_Global_velocity())*-0.5f;
			...
			if (lejoueur[j]->etat==true)
			{
								...
								if (j==VRAI)
								{
											...
											if (j==VRAI) //c moi
											{
												...
												if (lejoueur[j]->vie<=0)
												{
													...
													//** CFT ici on simule le confirme tue pour replacer le flag si on s'est fait sauter
													//** CFT ca ne suffit pas !! CFT_replace_flag(lejoueur[j]->ID);
													
													//** CFT A VOIR en fait il faudrait remettre le confirme tue mm pour au moins avertir qu'un joueur a ete flingue
				...
				if (j!=VRAI)
				{
					...
					if (lejoueur[j]->Visible==true && (m_overlay==true) && (HORS_DE_LA_MAP==false))
					{
						...
						if (v.len()<700.0f)
						{
							if((est_gign(lejoueur[j]->id_modele) && est_gign(lejoueur[VRAI]->id_modele)) || (!est_gign(lejoueur[j]->id_modele) && !est_gign(lejoueur[VRAI]->id_modele)))
...
*/
	...
	//	memcpy( &g_dsListenerParams.vVelocity, &Vel, sizeof(D3DVECTOR) );
		//c3_t lapos=m_camera.eye();
			...
			if(old_pos[0]!=lejoueur[VRAI]->pos[0] || old_pos[1]!=lejoueur[VRAI]->pos[1] || old_pos[2]!=lejoueur[VRAI]->pos[2])
			{
...
if (((GetTickCount()-old_pas)>300) && strcmp("run",lejoueur[VRAI]->dep)==0) 
{
	...
	if ((lejoueur[VRAI]->tir && lejoueur[VRAI]->fire ==true )) //  
	{
		...
		DecaleArme();
					...
					*/
		if (!strcmp(lejoueur[VRAI]->arme.name,"grenade") || !strcmp(lejoueur[VRAI]->arme.name,"c4")  || !strcmp(lejoueur[VRAI]->arme.name,"fumigene")  || !strcmp(lejoueur[VRAI]->arme.name,"plasma") ) //
		{
			bool ok=false;
			...
			world.process_visible_faces_collide(col_mur.pt+col_mur.normal,col_mur.normal);
				...
				if(col_enface.found)
				{
					grenades->SystemNew(new   pGrenSystemLazer_t , col_mur.pt,(col_enface.pt-col_mur.pt), 1,g_dpnidLocalPlayer);
					ok=true;
					pos_gre=col_mur.pt;
			...
			else
			{
				...
				{
					grenades->SystemNew(new   pGrenSystemSimple_t , lejoueur[VRAI]->pos,(flash2), 1,g_dpnidLocalPlayer);
				...
				else
				{
					...
					{
						grenades->SystemNew(new   pGrenSystemSmoke_t , lejoueur[VRAI]->pos,(flash2), 1,g_dpnidLocalPlayer);
						ok=true;
					...
					else
					{
						...
						if (exterieur)
						{
							//grenades->SystemNew(new   pGrenSystemMissile_t , lejoueur[VRAI]->pos,(flash2), 1,true);
		...
		else
		{
		 ...
		 //	world.process_visible_faces(m_camera);
			...
			else
			{
				...
				if (!sniper)
				{
					parts2->SystemNew(new   pSystemBurst_t,lejoueur[VRAI]->vue.StudioCalcAttachments (true), 1); // particle canon
				}
...
//			rr.normalize();
	//		rr=rr*0.7f;
		//	grenades->SystemNew(new   pGrenSystemMissile_t , lejoueur[VRAI]->m_PhysEnv->AxeG, rr, 1,g_dpnidLocalPlayer);
...
if(modechat && !menu_mode)	
{
		...
		lesinput_box[9]->frame(m_input.xpos,m_input.ypos,m_width,m_height,m_input.left_button,config.isdebug,delta);
}
...
{
	lockequipe();//----------------------------------------------LOCK	
 m_pings->print(m_width,m_height,&lejoueur,CFT_nb_gign,CFT_nb_terro,CFT_ON,TEAM_ON,CFT_eta,CFT_count,cft_timing,FRIENDLY,SNIPER_ON);
	unlockequipe();//----------------------------------------------LOCK
...
for(int da=0;da<lesconsoles.size();da++)
{
	if (lesconsoles[da])   
	{   
	lesconsoles[da]->visible=menu_mode && (
							((da==0 || da==2 ) && menu_state==1100) //|| da==1
							||
							((da==0 || da==2) && (menu_state==0 || menu_state==444 || menu_state==555|| menu_state==990 ))
							||
							((da==0 || da==2 ) && (menu_state==64 ))
							||
							((da==0 || da==2) &&  (menu_state==51 ))
							||
							((da==0 || da==3 || da==15) && (menu_state==2 ))
							||
							((da==0 || da==4) && (menu_state==110))
							||
							((da==16 ) && (menu_state==0))
							||
							((da==0 || da==444 || da==5 || da==6 || da==7 || da==8 || da==9 ) && (menu_state==151))//(da==10 && PASS_ON) ||
							||
							((da==0) && (menu_state==0 ))
							||
							((da==12 || da==2) &&  menu_state==62 )
							||
							((da==0 || da==2) &&  menu_state==2565 ) //confirmation logout
							||
							((da==12) && ( menu_state==2052|| menu_state==51|| menu_state==64 || menu_state==63|| (menu_state==351 && tournoisv.size()==0 )))
							) || ((da==13) && (m_tchat>0.0f));
...
for(int da=0;da<lesinput_box.size();da++)
{
	...
	if (lesinput_box[da] )   
	{   
	lesinput_box[da]->visible=menu_mode && (
							((da==0 || da==1) && menu_state==444)
							||
...
lesinput_box[da]->actif=active_input==da;
	...
	if (lesinput_box[da]->visible)	
				lesinput_box[da]->frame(m_input.xpos,m_input.ypos,m_width,m_height,m_input.left_button,config.isdebug,delta);
   ...
   for(int dao=0;dao<lesoptionsbouton.size();dao++)
	{
		...
		/*if (lesoptionsbouton[dao]->le_btn.m_mouseclick)
			...
			*/
			...
			lesoptionsbouton[dao]->frame(m_input.xpos,m_input.ypos,m_width,m_height,m_input.left_button,config.isdebug,delta);
...
void Engine::addblood(vec3_t pos,vec3_t dir)
{
	...
	if (menu_state==51  || menu_state==64)
	{
		...
		for(int y=0;y<3;y++)
		{
			...
			rnd_v=vec3_t(random_t::RandomRange(-1.0f, 1.0f),random_t::RandomRange(-1.0f, 1.0f),random_t::RandomRange(-1.0f, 1.0f));
...
/*HRESULT Engine::colle_position(DPNID idplayer,vec3_t  pos,vec3_t  rot,float  ang_dos,float fps,int id_model,int id_arme,char * dep ,char * act,int sens)
{
//reception
...
*/
        ...
        m_pDP->SendTo( DPNID_ALL_PLAYERS_GROUP, &bufferDesc, 1,100, NULL , &hAsync, DPNSEND_NOLOOPBACK | DPNSEND_NOCOMPLETE | DPNSEND_PRIORITY_LOW );
    }
...
HRESULT WINAPI Engine::MessageHandler( PVOID pvUserContext,
                                                  DWORD dwMessageId,
                                                  PVOID pMsgBuffer )
{
	...
	*/
	  switch(dwMessageId) 
    {
      ...
      pdpMsgIndicateConnect = (DPNMSG_INDICATE_CONNECT *) pMsgBuffer;
			...
			pServerConnectData = (TServerConnectData *) HeapAlloc (GetProcessHeap (),
																 0,
																 sizeof (TServerConnectData));
		  strcpy (pServerConnectData->_cData,
				  m_cur_map);
			strcpy (pServerConnectData->_Host,
				  m_cur_host);
    ...
    case DPN_MSGID_RETURN_BUFFER:
        ...
        case DPN_MSGID_CONNECT_COMPLETE:
        {
			...
			case DPNERR_ADDRESSING			: m_chat->addtext("error dplay",2); break;
			case	DPNERR_ALREADYCLOSING		: m_chat->addtext("error dplay",2); break;
			case DPNERR_ALREADYCONNECTED		: m_chat->addtext("error dplay",2); break;
			case DPNERR_ALREADYDISCONNECTING	: m_chat->addtext("error dplay",2); break;
			case DPNERR_ALREADYINITIALIZED	: m_chat->addtext("error dplay",2); break;
			case DPNERR_ALREADYREGISTERED	: m_chat->addtext("error dplay",2); break;
			case DPNERR_BUFFERTOOSMALL		: m_chat->addtext("error dplay",2); break;
			case DPNERR_CANNOTCANCEL			: m_chat->addtext("error dplay",2); break;
			case DPNERR_CANTCREATEGROUP		: m_chat->addtext("error dplay",2); break;
			case DPNERR_CANTCREATEPLAYER		: m_chat->addtext("error dplay",2); break;
case DPNERR_CANTLAUNCHAPPLICATION: m_chat->addtext("error dplay",2); break;
case DPNERR_CONNECTIONLOST	: m_chat->addtext("error dplay",2); break;
			case DPNERR_CONVERSION			: m_chat->addtext("error dplay",2); break;
			case	DPNERR_DATATOOLARGE			: m_chat->addtext("error dplay",2); break;
			case DPNERR_DOESNOTEXIST			: m_chat->addtext("error dplay",2); break;
			case DPNERR_DUPLICATECOMMAND		: m_chat->addtext("error dplay",2); break;
			case DPNERR_ENDPOINTNOTRECEIVING	: m_chat->addtext("error dplay",2); break;
			case	DPNERR_ENUMQUERYTOOLARGE	: m_chat->addtext("error dplay",2); break;
			case	DPNERR_ENUMRESPONSETOOLARGE		: m_chat->addtext("error dplay",2); break;
			case DPNERR_EXCEPTION			: m_chat->addtext("error dplay",2); break;
			case DPNERR_GENERIC				: m_chat->addtext("error dplay",2); break;
			case DPNERR_GROUPNOTEMPTY		: m_chat->addtext("error dplay",2); break;
			case DPNERR_HOSTING                 : m_chat->addtext("error dplay",2); break;
			case DPNERR_HOSTREJECTEDCONNECTION: m_chat->addtext("error dplay",2); break;
			case DPNERR_HOSTTERMINATEDSESSION: m_chat->addtext("error dplay",2); break;
			case DPNERR_INCOMPLETEADDRESS	: m_chat->addtext("error dplay",2); break;
			case DPNERR_INVALIDADDRESSFORMAT	: m_chat->addtext("error dplay",2); break;
			case DPNERR_INVALIDAPPLICATION		: m_chat->addtext("error dplay",2); break;
			case DPNERR_INVALIDCOMMAND		: m_chat->addtext("error dplay",2); break;
			case DPNERR_INVALIDDEVICEADDRESS		: m_chat->addtext("error dplay",2); break;
			case DPNERR_INVALIDENDPOINT		: m_chat->addtext("error dplay",2); break;
			case DPNERR_INVALIDFLAGS			: m_chat->addtext("error dplay",2); break;
			case DPNERR_INVALIDGROUP			 : m_chat->addtext("error dplay",2); break;
			case DPNERR_INVALIDHANDLE			: m_chat->addtext("error dplay",2); break;
			case DPNERR_INVALIDHOSTADDRESS	: m_chat->addtext("error dplay",2); break;
			case DPNERR_INVALIDINSTANCE		: m_chat->addtext("error dplay",2); break;
			case DPNERR_INVALIDINTERFACE		: m_chat->addtext("error dplay",2); break;
			case DPNERR_INVALIDOBJECT		: m_chat->addtext("error dplay",2); break;
			case DPNERR_INVALIDPARAM			: m_chat->addtext("error dplay",2); break;
			case DPNERR_INVALIDPASSWORD		: m_chat->addtext("mot de passe invalide",2); break;
			case DPNERR_INVALIDPLAYER		: m_chat->addtext("error dplay",2); break;
case DPNERR_INVALIDPOINTER	: m_chat->addtext("error dplay",2); break;
			case DPNERR_INVALIDPRIORITY		: m_chat->addtext("error dplay",2); break;
			case DPNERR_INVALIDSTRING	: m_chat->addtext("error dplay",2); break;
case DPNERR_INVALIDURL			: m_chat->addtext("error dplay",2); break;
			case DPNERR_NOCAPS				: m_chat->addtext("error dplay",2); break;
			case DPNERR_NOCONNECTION			: m_chat->addtext("error dplay",2); break;
			case DPNERR_NOHOSTPLAYER			: m_chat->addtext("error dplay",2); break;
			case DPNERR_NOINTERFACE			: m_chat->addtext("error dplay",2); break;
			case DPNERR_NOMOREADDRESSCOMPONENTS	: m_chat->addtext("error dplay",2); break;
			case DPNERR_NORESPONSE			: m_chat->addtext("DPNERR_NORESPONSE",2); break;
			case DPNERR_NOTALLOWED			: m_chat->addtext("error dplay",2); break;
			case DPNERR_NOTHOST				: m_chat->addtext("error dplay",2); break;
			case DPNERR_NOTREADY				: m_chat->addtext("error dplay",2); break;
			case DPNERR_NOTREGISTERED: m_chat->addtext("error dplay",2); break;
			case DPNERR_OUTOFMEMORY	: m_chat->addtext("error dplay",2); break;
			case DPNERR_PENDING	: m_chat->addtext("error dplay",2); break;
			case DPNERR_PLAYERALREADYINGROUP: m_chat->addtext("error dplay",2); break;
			case DPNERR_PLAYERLOST: m_chat->addtext("error dplay",2); break;
			case DPNERR_PLAYERNOTINGROUP: m_chat->addtext("error dplay",2); break;
			case	DPNERR_PLAYERNOTREACHABLE: m_chat->addtext("DPNERR_PLAYERNOTREACHABLE",2); break;
			case DPNERR_SENDTOOLARGE: m_chat->addtext("error dplay",2); break;
			case DPNERR_SESSIONFULL: m_chat->addtext("error dplay",2); break;
			case DPNERR_TABLEFULL: m_chat->addtext("error dplay",2); break;
			case DPNERR_TIMEDOUT: m_chat->addtext("error dplay",2); break;
			case DPNERR_UNINITIALIZED: m_chat->addtext("error dplay",2); break;
			case DPNERR_UNSUPPORTED: m_chat->addtext("error dplay",2); break;
			case DPNERR_USERCANCEL: m_chat->addtext("error dplay",2); break;
			}
...
//if (pConnectCompleteMsg->hResultCode!= -2146073472 && pConnectCompleteMsg->hResultCode!=DPNERR_PLAYERNOTREACHABLE )
...
HRESULT Engine::SessionsDlgNoteEnumResponse( PDPNMSG_ENUM_HOSTS_RESPONSE pEnumHostsResponseMsg )
{
    ...
    {wsprintf( szSessionTemp, TEXT("%s (%d/%d) (%dms) %s"), strName,
...
//	lagg_ms=pEnumHostsResponseMsg->dwRoundTripLatencyMS; // update du lagg
    ...
    {
        wsprintf( szSessionTemp, TEXT("%s (%d) (%dms)  %s"), strName,
                  pResponseMsgAppDesc->dwCurrentPlayers,
                  pEnumHostsResponseMsg->dwRoundTripLatencyMS,pstrName );
    }
...
*/
        ...
        while ( pDPHostEnum != &m_DPHostEnumHead )
        {
			...
			if(m_input.xpos >xd && m_input.xpos<xd*3 && m_input.ypos >yd*3-numb*20-100 && m_input.ypos<yd*3-(numb-1)*20-100)
			{//quit
					glColor4f(0.0f,0.3f,1.0f,0.5f);
...
void 
Engine::Joueur_Creation(char * playername,DPNID ID,long cle_joueur)
{
	...
	if (ID==g_dpnidLocalPlayer)
	{
		...
		if((m_bHostPlayer == TRUE ))
		{
			...
			{
				m_serverid=m_xmlsession.DevenirServer (m_playerfile,m_cur_map,( CFT_ON | (TEAM_ON<<1) | (SNIPER_ON<<2)),m_dwMaxPlayers,cle_tournois,round,cur_md5,lesinput_box[6]->text.text,PASS_ON,les_armes_autorisees);//
				idpartie=m_xmlsession.JoinServer(m_playerfile,m_serverid);
			}	
			menu_state=51;
		}
...
SECURITY_ATTRIBUTES sec;
       ...
       if ( CreateProcess(file, NULL, &sec, &sec, TRUE, 
                        0, NULL, NULL,  &start, &pinfo))
	   {
...
bool existe=false;
	...
	//	MessageBox (m_hwnd,TEXT("Attention download de la map. Cela va prendre quelques secondes ..."),TEXT("ROMUSTRIKE"),MB_OK);
		//sinon dwlolad
		//dezip
		//exit
...
void 
Engine::JoueUnSon(int id,vec3_t lapos)
{
...
{
	if ((p[1]>=mins[1])&&(p[1]<=maxs[1])&&(p[2]>=mins[2])&&(p[2]<=maxs[2])) return true;
	else return false;
}

bool world_t::pip_const_y(const vec3_t& p,const short* mins, const short* maxs)
{
	if ((p[0]>=mins[0])&&(p[0]<=maxs[0])&&(p[2]>=mins[2])&&(p[2]<=maxs[2])) return true;
	else return false;
}

bool world_t::pip_const_z(const vec3_t& p,const short* mins, const short* maxs)
{
	if ((p[0]>=mins[0])&&(p[0]<=maxs[0])&&(p[1]>=mins[1])&&(p[1]<=maxs[1])) return true;
	else return false;
}
...
bool world_t::intersectbox(const vec3_t& p1,const vec3_t& p2,const short* vmin, const short* vmax)
{
     vec3_t s(p1);
     vec3_t e(p2);
     if (((s[0]>=vmin[0]) && (s[1]>=vmin[1]) && (s[2]>=vmin[2]) &&
          (s[0]<=vmax[0]) && (s[1]<=vmax[1]) && (s[2]<=vmax[2])) ||
          ((e[0]>=vmin[0]) && (e[1]>=vmin[1]) && (e[2]>=vmin[2]) &&
          (e[0]<=vmax[0]) && (e[1]<=vmax[1]) && (e[2]<=vmax[2])))
     {
         return true;
     } else {
         ...
         for (i=0; i<6; i++) {
             switch (i) {
                 ...
                 case 0:
                     if (isect_const_xyz(0,vmin[0],s,e,out) && pip_const_x(out,vmin,vmax)) return true;
                     break;
                 case 1:
                     if (isect_const_xyz(0,vmax[0],s,e,out) && pip_const_x(out,vmin,vmax)) return true;
                     break;
                 case 2:
                     if (isect_const_xyz(1,vmin[1],s,e,out) && pip_const_y(out,vmin,vmax)) return true;
                     break;
                 case 3:
                     if (isect_const_xyz(1,vmax[1],s,e,out) && pip_const_y(out,vmin,vmax)) return true;
                     break;
                 case 4:
                     if (isect_const_xyz(2,vmin[2],s,e,out) && pip_const_z(out,vmin,vmax)) return true;
                     break;
                 case 5:
                     if (isect_const_xyz(2,vmax[2],s,e,out) && pip_const_z(out,vmin,vmax)) return true;
                     break;
             }
         }
     }
   ...
   if( g_lNumberOfActivePlayers > 1 )
    {
        ...
        m_pDP->SendTo( DPNID_ALL_PLAYERS_GROUP, &bufferDesc, 1,
                       100, NULL, &hAsync, DPNSEND_NOLOOPBACK | DPNSEND_NOCOMPLETE | DPNSEND_PRIORITY_LOW );
    }
		...
		if (ret->pos[0]!=0.0f)
		{
			...
			for(int j = 0; j < g_lNumberOfActivePlayers; j++)
			{
					if (lejoueur[j]->ID==idplayer && j!=VRAI)
					{
							...
							lapos.y =lejoueur[j]->pos[1];
							lapos.z =lejoueur[j]->pos[2];
							if(lejoueur[j]->id_arme_recu==1 || lejoueur[j]->id_arme_recu==4 || lejoueur[j]->id_arme_recu==6 || lejoueur[j]->id_arme_recu==7 || lejoueur[j]->id_arme_recu==8)
							{
   ...
   if( g_lNumberOfActivePlayers > 1 )
    {
        ...
        DPNHANDLE hAsync;
        m_pDP->SendTo(dpnidPlayer, &bufferDesc, 1,
                       0, NULL, &hAsync,DPNSEND_GUARANTEED );
    }
...
*/
   ...
   // strcpy(,"rere");//text
    ...
    DPN_BUFFER_DESC bufferDesc;
    bufferDesc.dwBufferSize = sizeof(msgChat.dwType) + 
                              (strlen(msgChat.strChatString)+1)*sizeof(TCHAR);
...
HRESULT Engine::Confirme_Tue(DPNID dpnidPlayer,float dans_la_tete)
{
   ...
   if( g_lNumberOfActivePlayers > 0 )
	...
	//	g_camera_suivi=dpnidPlayer;
        ...
        DPNHANDLE hAsync;
		m_pDP->SendTo( DPNID_ALL_PLAYERS_GROUP, &bufferDesc, 1,
                   0, NULL, &hAsync, DPNSEND_GUARANTEED | DPNSEND_PRIORITY_HIGH);
   ...
   if( g_lNumberOfActivePlayers > 0 )
    {
        ...
        DPNHANDLE hAsync;
		m_pDP->SendTo( DPNID_ALL_PLAYERS_GROUP, &bufferDesc, 1,
                   0, NULL, &hAsync, DPNSEND_PRIORITY_LOW);
	...
	for(int j = 0; j < g_lNumberOfActivePlayers; j++)
		{
			if (lejoueur[j]->etat==true && j!=VRAI && lejoueur[j]->Visible ==true )
			{
				...
				// on tape dans la sphere
					...
					if (last_id!=-1)
					{
						dist_joueur= fabs(sqrt(pow(src[0]-lejoueur[j]->modele.pos_touche[0],2)+pow(src[1]-lejoueur[j]->modele.pos_touche[1],2)));
					//	dist_joueur= abs(sqrt(pow(dest[0]-lejoueur[j]->modele.pos_touche[0],2)+pow(dest[1]-lejoueur[j]->modele.pos_touche[1],2)+pow(dest[2]-lejoueur[j]->modele.pos_touche[2],2)));
...
void world_t::DessineSkyBox(vec3_t pos)
{
	...
	*/
	...
	// face arriere
		glTexCoord2i(-1,0);glVertex3f(x+SKYtaille/2.0f,y-SKYtaille/2.0f,z-SKYtaille/2.0f);
		glTexCoord2i(0,0);glVertex3f(x-SKYtaille/2.0f,y-SKYtaille/2.0f,z-SKYtaille/2.0f);
		glTexCoord2i(0,1);glVertex3f(x-SKYtaille/2.0f,y+SKYtaille/2.0f,z-SKYtaille/2.0f);
		glTexCoord2i(-1,1);glVertex3f(x+SKYtaille/2.0f,y+SKYtaille/2.0f,z-SKYtaille/2.0f);
	glEnd();
		...
		if (m_playerfile && !lan_mode)
		{
			...
			if ( (le_socket!=0) &&(!(strlen(m_playerfile->player_name)==0)))
			{
				...
				if (g_lNumberOfActivePlayers >0)
				{
					if (menu_state==62)
					{
						sprintf(chaine,"<who><pseudo>%s</pseudo><site>CNX JOUEUR %i</site><score>%i</score></who>\0",m_playerfile->player_name,g_lNumberOfActivePlayers,m_playerfile->score);
					...
					else
					{
						...
						sprintf(chaine,"<who><pseudo>%s</pseudo><site>Partie en cours v%d</site><map>%s %s</map><score>%i</score><life>%i</life><id_player>%i</id_player><host>%i</host><MODE>%i</MODE><DESARME>%s</DESARME><ECOUTE>%s</ECOUTE><STORM>%s</STORM></who>\0",m_playerfile->player_name ,m_xmlsession.Version_Soft,m_cur_map,mode_jeu,lescore,vie,m_playerfile->player_id,m_bHostPlayer, CFT_ON | (TEAM_ON<<1),desarme,sur_ecoute,sto);
					}
				...
				else
				{
					...
					sprintf(chaine,"<who><pseudo>%s</pseudo><site>%s</site><score>%i</score></who>\0",m_playerfile->player_name,title,m_playerfile->score);
				 	if (downloading==INET_CHUNK)
					{
						 int pct=((100*total_down)/m_dwTotalSize);

						sprintf(chaine,"<who><pseudo>%s</pseudo><site>Dans le jeu</site><map>%s %i/100</map><score>%i</score><life>0</life></who>\0",m_playerfile->player_name,m_cur_map,pct,m_playerfile->score);
					}
...
HRESULT Engine::envoi_msg_ops(char*msg)
{
	if (m_playerfile)
	{
		...
		{
		char chaine[SIZE_CHAINE];
		sprintf(chaine,"<racine><pseudo>%s</pseudo><msg>%s</msg><priv>oui</priv></racine>\0",m_playerfile->player_name,msg);
		send(le_socket,(LPCTSTR)chaine,strlen(chaine)+1,NULL);
		}
	}
...
*/
   ...
   {
      for (ent_index = 0; ent_index < bsp->num_entities; ent_index++)
      {
 
		  entity_t mon_ent=bsp->Lesentities[ent_index];
		...
		if (value[0])
		{
			...
			if ((strcmp(bsp->ValueForKey(&bsp->Lesentities[ent_index],"classname"),"func_breakable")==0))
			{
				continue;
			}
			if ((strcmp(bsp->ValueForKey(&bsp->Lesentities[ent_index],"classname"),"func_door")==0))
			{
				continue;
			 }
			if ((strcmp(bsp->ValueForKey(&bsp->Lesentities[ent_index],"classname"),"func_door_rotating")==0))
			{
				continue;
			 }
			if ((strcmp(bsp->ValueForKey(&bsp->Lesentities[ent_index],"classname"),"trigger_multiple")==0))
			{
				continue;
			 }
			...
			value = bsp->ValueForKey(&bsp->Lesentities[ent_index], "model");
			//o<<"debut"<<value<<endl; 
				}
			if (!value[0])
				continue;
            ...
            enttmp[0].rendermode = 0;
            enttmp[0].renderamt = 1.0f;
			value = bsp->ValueForKey(&bsp->Lesentities[ent_index], "renderamt");
			if (value[0])
			{
				int  n_renderamt;

				sscanf(value, "%d", &n_renderamt);
				enttmp[0].renderamt = n_renderamt / 255.0f;
			}
			...
			if (!(((pass == 0) && (enttmp[0].renderamt == 1.0f)) ||
                ((pass == 1) && (enttmp[0].renderamt != 1.0f))))
				continue;
...
/*			value = bsp->ValueForKey(&bsp->Lesentities[ent_index], "classname");
			...
			*/
			...
			enttmp[0].origin[0] = 0.0f;
			enttmp[0].origin[1] = 0.0f;
			enttmp[0].origin[2] = 0.0f;
			enttmp[0].size_leaf=0;
			enttmp[0].leaf_visibility_list[0]=0;

			enttmp[0].angles[0] = 0.0f;
			enttmp[0].angles[1] = 0.0f;
			enttmp[0].angles[2] = 0.0f;

			enttmp[0].rendermode = 0;
			enttmp[0].renderamt = 1.0f;
			enttmp[0].rendercolor[0] = 1.0f;
			enttmp[0].rendercolor[1] = 1.0f;
			enttmp[0].rendercolor[2] = 1.0f;
			enttmp[0].renderfx = 0;

			enttmp[0].brush_model_index = -1;

			//enttmp[0].studio_model = NULL;
			...
			value = bsp->ValueForKey(&bsp->Lesentities[ent_index], "angle");
			if (value[0])
			{
				// set the yaw angle...
				sscanf(value, "%f", &enttmp[0].angles[1]);
			}
			...
			if (value[0])
			{
				...
				enttmp[0].rendercolor[0] = n_color_r / 255.0f;
				enttmp[0].rendercolor[1] = n_color_g / 255.0f;
				enttmp[0].rendercolor[2] = n_color_b / 255.0f;
			}
			 ...
			 {
				enttmp[0].brush_model_index=-1;
				sscanf(value, "*%d", &enttmp[0].brush_model_index);

				if (enttmp[0].brush_model_index>=0)
				{
				   dmodel_t *model;

				   // calculate the origin for this brush model...
				   model = &bsp->models[enttmp[0].brush_model_index];
		...
		*/
			...
			//ok on est bon on peux l'ajouter au tableau
			value = bsp->ValueForKey(&bsp->Lesentities[ent_index], "classname");
			value2 = bsp->ValueForKey(&bsp->Lesentities[ent_index], "model");

			if ( ( (enttmp[0].rendermode==2) || (enttmp[0].rendermode==5)))//false &&
			{
				EntVarInvi.push_back(enttmp[0]);
...
void world_t::render_skyfaces(vec3_t cam)
{
