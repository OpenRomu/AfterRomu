...
void pGrenadeManager_t::Frame(float &frametime ,vector<vec3_t>* lespos,vector<DPNID>* amoi,vector<vec3_t>* lesposjoueur,DPNID mon_id,vector<DPNID>* lesid_joueur,vector<Aplayer*>* lesjoueurs,vector<CPhysEnv* > lescars)
{
   ...
   while (curr)
   {
      ...
      if (!(ok = curr->Frame(frametime, gravity, lespos,amoi,lesposjoueur,mon_id,lesid_joueur,lesjoueurs,lescars)))
	  {
		  ...
		  for (part = curr->GetAlive()->next; part != curr->GetAlive();part = part->next)
			{
			 ...
			 if (part->life!=1.0f)
			 {
				...
				if (col_tir.found==true)
				{
					if (part->rebond)
					{
						if ((col_tir.normal[0]==0.0f) && (col_tir.normal[1]==0.0f) &&(col_tir.normal[2]==0.0f))
						{
							part->pos =part->pold;
							part->gravity =0;
