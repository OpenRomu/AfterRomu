...
: xres(640), yres(480),msens(120), bpp(16), hz(60), vsync_off(false),fullscreen(false),isdebug(false),GAMMA(5),music_on(true),viseur(10)
{
...
sprintf(server_tchat,"romu.soft.free.fr");
    ...
    for(int i = 0; i < tokens.size(); i++) {
        ...
        case 1: {
                    switch(disp_count++) {
                    case 0: xres = atoi(tokens[i].c_str()); break;
                    case 1: yres = atoi(tokens[i].c_str()); break;
                    case 2: bpp = atoi(tokens[i].c_str()); break;
                    case 3: hz = atoi(tokens[i].c_str()); break;
                    case 4: if(tokens[i] == "true" || tokens[i] == "TRUE") {
                                vsync_off = true;
                            }
                            break;
					case 5: if(tokens[i] == "true" || tokens[i] == "TRUE") {
                                fullscreen = true;
                            }
                            break;
                    }
...
void config_t::save()
{
	 try {
		...
		if(f) {
			...
			sprintf(tmp,"[debug]\n%s\n[gamma]\n%d\n[display]\n%d,%d,%d,%d,%s,%s\n[keys]\n%s\n[server_xml]\n%s\n[server_tchat]\n%s\n[mouse]\n%d\n[music_on]\n%s\n[viseur]\n%d\n",kdebug,GAMMA,xres,yres,bpp,hz,kvsync,kfull,touche,server_xml,server_tchat,msens,kmusic_on,viseur);
