...
void*
bsp_file_t::suck_lump(FILE* f, lump_t& l, int sz, int& count)
{
...
: model_count(0), models(0),
  face_count(0), faces(0),
  plane_count(0), planes(0),
  edge_count(0), edges(0),
  edge_list_count(0), edge_list(0),
  vertex_count(0), vertices(0),
  texinfo_count(0), texinfo(0),
  tex_count(0), textures(0),
  lightmap_count(0), lightmaps(0),
  node_count(0), nodes(0),
  clipnode_count(0), clipnodes(0),
  leaf_count(0), leaves(0),
  marksurface_count(0), marksurfaces(0),
  vis_count(0), vis(0),
  entities_count(0), entities(0)
{
...
bsp_file_t::~bsp_file_t()
{
    ...
    if(planes) { delete planes, planes = 0; }
    if(edges) { delete edges, edges = 0; }
    if(edge_list) { delete edge_list, edge_list = 0; }
    if(vertices) { delete vertices, vertices = 0; }
    if(texinfo) { delete texinfo, texinfo = 0; }
    if(textures) { delete textures, textures = 0; }
    if(lightmaps) { delete lightmaps, lightmaps = 0; }
    if(nodes) { delete nodes, nodes = 0; }
    if(clipnodes) { delete clipnodes, clipnodes = 0; }
    if(leaves) { delete leaves, leaves = 0; }
    if(marksurfaces) { delete marksurfaces, marksurfaces = 0; }
    if(models) { delete models, models = 0; }
    if(vis) { delete vis, vis = 0; }
    if(entities) { delete entities, entities = 0; }
