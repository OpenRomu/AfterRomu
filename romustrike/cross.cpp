...
void
Cross::print(int m_width,int m_height)

{
  ...
  // glEnable(GL_BLEND);
	...
	glBegin(GL_QUADS);											// Start Drawing A Quad
		...
		//if (courante==1)
		//{
			glTexCoord2fv(m_coords[(int)id]->c00); glVertex3f(m_width/2-WW/2,m_height/2-HH/2,0.0f);	// Bottom Left
			glTexCoord2fv(m_coords[(int)id]->c10); glVertex3f( m_width/2+WW/2,m_height/2-HH/2,0.0f);	// Bottom Right
			glTexCoord2fv(m_coords[(int)id]->c11); glVertex3f( m_width/2+WW/2, m_height/2+HH/2,0.0f);	// Top Right
			glTexCoord2fv(m_coords[(int)id]->c01); glVertex3f(m_width/2-WW/2, m_height/2+HH/2,0.0f);	// Top Left
		//}
		//	else
		//{
	...
	if (courante==1)
	{
		...
		glBegin(GL_QUADS);											// Start Drawing A Quad
			...
			glTexCoord2fv(c00); glVertex3f(0,0,0.0f);	// Bottom Left
			glTexCoord2fv(c10); glVertex3f( (m_width-m_height)/2,0,0.0f);	// Bottom Right
			glTexCoord2fv(c11); glVertex3f( (m_width-m_height)/2, m_height,0.0f);	// Top Right
			glTexCoord2fv(c01); glVertex3f(0, m_height,0.0f);	// Top Left
		...
		glBegin(GL_QUADS);											// Start Drawing A Quad
			...
			glTexCoord2fv(c00); glVertex3f(m_width-(m_width-m_height)/2,0,0.0f);	// Bottom Left
			glTexCoord2fv(c10); glVertex3f( m_width,0,0.0f);	// Bottom Right
			glTexCoord2fv(c11); glVertex3f( m_width, m_height,0.0f);	// Top Right
			glTexCoord2fv(c01); glVertex3f(m_width-(m_width-m_height)/2, m_height,0.0f);	// Top Left
