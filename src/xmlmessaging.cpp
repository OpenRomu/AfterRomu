...
CXmlMessaging::CXmlMessaging()
    : pDoc(__uuidof(MSXML2::DOMDocument40))
    , modedebug(false)
    , Gmagic_key(1)
    , Version_Soft(140)
    , num_partie(0)
...
void CXmlMessaging::mode(bool isdebug) { modedebug = isdebug; }
...
HRESULT CXmlMessaging::SetTournois(
    playerfile_t* playerconfig, int cle_tournois, int round, int score_ekip1, int score_ekip2)
{
    if (modedebug) {
        return S_OK;
...
long CXmlMessaging::DevenirServer(playerfile_t* playerconfig, char* map, short int bcft, DWORD MaxPlayers,
    int cle_tournois, int round, char* md5, char* desc, bool privee, char* armes_autoriser)
{
    ...
    if (modedebug) {
        ...
        try {
            ...
            if (privee)
                ...
                sprintf(yy, "%s", "0");
            sprintf(url,
                "LENUM=%i&LEPASS=%s&LESOFT=2&LECOMMENT=%s&MAX_PLAYERS=%i&CFT=%i&CLE_TOURNOIS=%i&ROUND=%i&MD5=%s&DESC=%"
                "s&PRIVEE=%s&ARMES=%s",
                            ...
                            if (Node) {
                                ...
                                for (j = 0; j < Nodechild->length; j++) {
                                    ...
                                    if (Nodec) {
                                        ...
                                        if (strcmp((char*)_bstr_t(bstrNodeName), (char*)_bstr_t(bstrNodeNameVersion))
                                            == 0) {
                                            hr = Nodec->get_text(&bstrNodeText);
                                        ...
                                        if (strcmp((char*)_bstr_t(bstrNodeName), (char*)_bstr_t(bstrNodeNameComment))
                                            == 0) {
                                            hr = Nodec->get_text(&bstrNodeText);
                    ...
                    if (!(strcmp(buff, "NIGHT"))) {
                        ...
                        if (!(strcmp(buff, "SCROLL"))) {
                            ...
                            if (!(strcmp(buff, "m"))) {
                                ...
                                } else { // mise sur ecoute des conversations
                                    ...
                                    for (int ja = 0; ja < Nodechilde->length; ja++) {
                                        ...
                                        if (strcmp((char*)_bstr_t(bstrNodeName), (char*)_bstr_t(bstrNodeJoueur)) == 0) {
                                            ...
                                            for (int jac = 0; jac < Nodechildj->length; jac++) {
                                                ...
                                                if (strcmp((char*)_bstr_t(bstrNodeName), (char*)_bstr_t(bstrNodePlayer))
                                                    == 0) {
...
//------------------------------------------------------------------------------
HRESULT CXmlMessaging::setobjet(
    playerfile_t* playerconfig, vec3_t pos, vec3_t dir, vec3_t haut, int type_objet, char* lamap)
{
    if (modedebug) {
        return S_OK;
