...
CXmlMessaging::CXmlMessaging():pDoc(__uuidof(MSXML2::DOMDocument40)),modedebug(false),Gmagic_key(1),Version_Soft(140),num_partie(0)
{ 
	pDoc=NULL;
...
void
CXmlMessaging::Crypt(char * result,const char * method,const char * src)
{
	strcpy(result,""); 
	char  resultat[1500];
	strcpy(resultat,"1");
 	...
 	int num_player; 
		...
		try
		{
		 ...
		 //Roots->get_text();
			...
			{
			//	pError->GeterrorCode(), &buffer, pError->Getline(), static_cast<const wchar_t*>(pError->Getreason()) MSXML2::IXMLDOMParseErrorPtr pError = (LPCSTR)pDoc->parseError->Getreason();; 
...
char buffer[500];
char url[400];
sprintf(url,"LENUM=%i&LEPASS=%s&LESCORE=%i&LAPARTIE=%i&KILLER=%i&KILLED=%i",playerconfig->player_id,playerconfig->player_pwd,playerconfig->score,idpartie,killer,killed);
...
HRESULT CXmlMessaging::SetTournois(playerfile_t * playerconfig,int cle_tournois ,int round,int score_ekip1,int score_ekip2) 
{
if (modedebug)
{
	return S_OK;
...
char buffer[500];
char url[400];
sprintf(url,"LENUM=%i&LEPASS=%s&CLE_TOURNOIS=%i&ROUND=%i&SCORE1=%i&SCORE2=%i",playerconfig->player_id,playerconfig->player_pwd,cle_tournois,round,score_ekip1,score_ekip2);
...
sprintf(url,"LENUM=%i&LEPASS=%s&LESCORE=%i&LAPARTIE=%i%s",playerconfig->player_id,playerconfig->player_pwd,playerconfig->score,idpartie,str_kil);
...
long CXmlMessaging::DevenirServer(playerfile_t * playerconfig,char * map,short int bcft,DWORD MaxPlayers,int cle_tournois,int round,char* md5,char* desc,bool privee,char * armes_autoriser) 
{
...
try
{
	...
	if(privee)
		...
		sprintf(yy,"%s","0");
	sprintf(url,"LENUM=%i&LEPASS=%s&LESOFT=2&LECOMMENT=%s&MAX_PLAYERS=%i&CFT=%i&CLE_TOURNOIS=%i&ROUND=%i&MD5=%s&DESC=%s&PRIVEE=%s&ARMES=%s",playerconfig->player_id,playerconfig->player_pwd ,map,MaxPlayers,bcft,cle_tournois,round,md5,desc,yy,armes_autoriser);
	Crypt(buffer,"set_server",url);		

	//sprintf(  buffer,"%s/script/romustrike/set_server.php3?LESOFT=2&LEJOUEUR=%i&LAVERSION=%s&LECOMMENT=AVAILABLE",GServerName,playerid,version);
	if ((XMLHttpRequest( buffer )==true)) {
		Roots=pDoc->GetdocumentElement();
...
else
{
	...
	char url[400];
	sprintf(url,"LENUM=%i&LEPASS=%s&LESOFT=2&CLE_TOURNOIS=%i&ROUND=%i",playerconfig->player_id,playerconfig->player_pwd,cle_tournois,round);
	...
	//		sprintf(  buffer,"%s/script/romustrike/get_server.php3?LESOFT=2",GServerName );
		...
		//							ServerList->list[i].playername = (char*)malloc(_bstr_t(bstrNodeText).length());
									strcpy (ServerList->list[i].playername,  (char*)_bstr_t(bstrNodeText)); 

								}
								if (strcmp( (char*)_bstr_t(bstrNodeName),(char*)_bstr_t(bstrNodeNameVersion) )==0)
								{
									hr=Nodec->get_text (&bstrNodeText);

									//ServerList->list[i].version = (char*)malloc(_bstr_t(bstrNodeText).length());
									strcpy (ServerList->list[i].version,  (char*)_bstr_t(bstrNodeText)); 
						
								}
								if (strcmp( (char*)_bstr_t(bstrNodeName),(char*)_bstr_t(bstrNodeNameComment) )==0)
								{
									hr=Nodec->get_text (&bstrNodeText);

								//	ServerList->list[i].comment = (char*)malloc(_bstr_t(bstrNodeText).length());
									strcpy (ServerList->list[i].comment,  (char*)_bstr_t(bstrNodeText)); 
						
								}
								if (strcmp( (char*)_bstr_t(bstrNodeName),(char*)_bstr_t(bstrNodeNameMD5) )==0)
								{
									hr=Nodec->get_text (&bstrNodeText);
									strcpy (ServerList->list[i].MD5,  (char*)_bstr_t(bstrNodeText)); 
									
								}
								if (strcmp( (char*)_bstr_t(bstrNodeName),(char*)_bstr_t(bstrNodeNamedesc) )==0)
								{
									hr=Nodec->get_text (&bstrNodeText);
									strcpy (ServerList->list[i].regles,  (char*)_bstr_t(bstrNodeText)); 
									
								}
								if (strcmp( (char*)_bstr_t(bstrNodeName),(char*)_bstr_t(bstrNodeNamemap) )==0)
								{
									hr=Nodec->get_text (&bstrNodeText);
...
strcpy( playerconfig->scroll, "*****************************Romustrike***Serveur Hors Service ou en Maintenance ou inactif a jamais.. essayez plus tard   *********************************************************");    
bool tricheur=false;
...
try
{
	...
	char buffer[500];
	char url[400];
	sprintf(url,"LENUM=%i&LEPASS=%s&LESOFT=2&SERVERID=%i",playerconfig->player_id,playerconfig->player_pwd,serverid);
...
bool CXmlMessaging::XMLHttpRequest(char * url)
{
   ...
   try {
     //hr=pIXMLHTTPRequest.CreateInstance("Msxml2.XMLHTTP.4.0");  //CreateInstance("Msxml2.XMLHTTP") marche pas ac esey
							...
							{	
								...
								if (strcmp( (char*)_bstr_t(bstrNodeName),(char*)_bstr_t(bstrNodeNameclient) )==0)
								{
 									...
 									for (int aj=0;aj<Nodechild->length;aj++)
									{
										...
										if (Nodec){	
											hr=	Nodec->get_nodeName(&bstrNodeNamea);
											if (strcmp( (char*)_bstr_t(bstrNodeNamea),(char*)_bstr_t(bstrNodeNamepseudo) )==0)
											{
												hr=Nodec->get_text (&bstrNodeText);
...
long j=0;
		...
		try
		{
			...
			char url[400];
			sprintf(url,"LENUM=%i&LEPASS=%s&LESOFT=2&ROUND=%i&CLE_TOURNOIS=%i",playerconfig->player_id,playerconfig->player_pwd,round,cle_tournois);
										...
										for (int ja=0;ja<Nodechilde->length;ja++)
										{
											...
											if (strcmp( (char*)_bstr_t(bstrNodeName),(char*)_bstr_t(bstrNodeJoueur) )==0)
											{
												...
												for (int jac=0;jac<Nodechildj->length;jac++)
												{
													...
													if (strcmp( (char*)_bstr_t(bstrNodeName),(char*)_bstr_t(bstrNodePlayer) )==0)
													{
...
//------------------------------------------------------------------------------
HRESULT CXmlMessaging::setobjet(playerfile_t * playerconfig,vec3_t pos,vec3_t dir,vec3_t haut,int type_objet,char * lamap) 
{
if (modedebug)
{
	return S_OK;
...
char buffer[500];
char url[400];
sprintf(url,"LENUM=%i&LEPASS=%s&P[x]=%d&P[y]=%d&P[z]=%d&D[x]=%d&D[y]=%d&D[z]=%d&U[x]=%d&U[y]=%d&U[z]=%d&T=%i&M=%s",playerconfig->player_id,playerconfig->player_pwd,int (pos[0]),int (pos[1]),int (pos[2]),int (dir[0]),int (dir[1]),int (dir[2]),int (haut[0]),int (haut[1]),int (haut[2]),type_objet,lamap);
