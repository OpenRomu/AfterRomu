...
static float intersect_sphere(const vec3_t& r, // point E
...
class Camera {
public:
    Camera(float x = 0.0f, float y = 0.0f, float z = 0.0f, float pitch = 0.0f, float roll = 0.0f, float yaw = 0.0f)
        : m_pitch(pitch)
        , m_roll(roll)
        , m_yaw(yaw)
    {
    ...
    inline void move_delta(float xd, float yd, float zd) { m_eye[0] += xd, m_eye[1] += yd, m_eye[2] += zd; }
    inline void move_delta(const vec3_t& v) { m_eye[0] += v[0], m_eye[1] += v[1], m_eye[2] += v[2]; }
    inline void move(float x, float y, float z) { m_eye[0] = x, m_eye[1] = y, m_eye[2] = z; }
    inline void move(const vec3_t& v) { m_eye[0] = v[0], m_eye[1] = v[1], m_eye[2] = v[2]; }
    inline void rotate_delta(float pitch_delta, float roll_delta, float yaw_delta)
    {
        m_pitch += pitch_delta;
        m_roll += roll_delta;
        m_yaw += yaw_delta;
    ...
    inline vec3_t right_vector() const { return vec3_t(cam[0], cam[4], cam[8]); }
    inline vec3_t up_vector() const { return vec3_t(cam[1], cam[5], cam[9]); }
    inline vec3_t fwd_vec() const { return vec3_t(cam[2], cam[6], cam[10]); }
