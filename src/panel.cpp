...
#include <GL/glut.h>
#include "fastmath.h"
Panel::Panel(const char* lesammo,const char* ammo,const char* smoke,const char* vie,const char* players ,const char* lazer ):courante(0),tps_players(0.0f),tps_vie(0.0f)
...
//	courante=m_tex[1]->id ;
   ...
   /* inc = 1.0f/4.0f;//0.0625f;
    ...
    m_coords[1]->c00[0] = 0; m_coords[1]->c00[1] = 0;
    m_coords[1]->c10[0] = 0+inc; m_coords[1]->c10[1] = 0;
    m_coords[1]->c11[0] = 0+inc, m_coords[1]->c11[1] = 0+inc;
    m_coords[1]->c01[0] = 0; m_coords[1]->c01[1] = 0+inc;
...
void
Panel::print(int m_width,int m_height,int nbammo,int vie,int nb_player,char * weapon)
	...
	int id;
		...
		if (strcmp(weapon,"plasma") && strcmp(weapon,"c4") && strcmp(weapon,"grenade") && strcmp(weapon,"fumigene") )
		{
			...
			id=0;
				...
				glTexCoord2fv(m_coords[(int)id]->c00); glVertex3f(20+WW+-WW/2+i*(WW/3),HH-HH/2,0.0f);	// Bottom Left
				glTexCoord2fv(m_coords[(int)id]->c10); glVertex3f(20+WW+ WW/2+i*(WW/3),HH-HH/2,0.0f);	// Bottom Right
				glTexCoord2fv(m_coords[(int)id]->c11); glVertex3f(20+WW+ WW/2+i*(WW/3), HH+HH/2,0.0f);	// Top Right
				glTexCoord2fv(m_coords[(int)id]->c01); glVertex3f(20+WW+-WW/2+i*(WW/3), HH+HH/2,0.0f);	// Top Left
				glEnd();
		...
		if (!strcmp(weapon,"grenade"))
		{
 			m_tex[0]->bind();
				glBegin(GL_QUADS);											// Start Drawing A Quad
				glTexCoord2fv(m_coords[(int)id]->c00); glVertex3f(20+WW+-WW/4+i*(WW/2),HH-HH/4,0.0f);	// Bottom Left
				glTexCoord2fv(m_coords[(int)id]->c10); glVertex3f(20+WW+ WW/4+i*(WW/2),HH-HH/4,0.0f);	// Bottom Right
				glTexCoord2fv(m_coords[(int)id]->c11); glVertex3f(20+WW+ WW/4+i*(WW/2), HH+HH/4,0.0f);	// Top Right
				glTexCoord2fv(m_coords[(int)id]->c01); glVertex3f(20+WW+-WW/4+i*(WW/2), HH+HH/4,0.0f);	// Top Left
				glEnd();
...
id=0;
		...
		if (!strcmp(weapon,"plasma"))
		{
			m_tex[4]->bind();
			glBegin(GL_QUADS);											// Start Drawing A Quad
				glTexCoord2fv(m_coords[(int)id]->c00); glVertex3f(20+WW+-WW/2+i*(WW/2),HH-HH/2,0.0f);	// Bottom Left
				glTexCoord2fv(m_coords[(int)id]->c10); glVertex3f(20+WW+ WW/2+i*(WW/2),HH-HH/2,0.0f);	// Bottom Right
				glTexCoord2fv(m_coords[(int)id]->c11); glVertex3f(20+WW+ WW/2+i*(WW/2),HH+HH/2,0.0f);	// Top Right
				glTexCoord2fv(m_coords[(int)id]->c01); glVertex3f(20+WW+-WW/2+i*(WW/2),HH+HH/2,0.0f);	// Top Left
				glEnd();
...
glBegin(GL_QUADS);											// Start Drawing A Quad
glTexCoord2fv(m_coords[(int)id]->c00); glVertex3f(X-H[2]/ratio,Y-W[2]/ratio,0.0f);	// Bottom Left
glTexCoord2fv(m_coords[(int)id]->c10); glVertex3f(X+H[2]/ratio,Y-W[2]/ratio,0.0f);	// Bottom Right
glTexCoord2fv(m_coords[(int)id]->c11); glVertex3f(X+H[2]/ratio,Y+W[2]/ratio,0.0f);	// Top Right
glTexCoord2fv(m_coords[(int)id]->c01); glVertex3f(X-H[2]/ratio,Y+W[2]/ratio,0.0f);	// Top Left
glEnd();
int XX=30;
int YY=70;
...
m_tex[3]->bind();
glBegin(GL_QUADS);											// Start Drawing A Quad
glTexCoord2fv(m_coords[(int)id]->c00); glVertex3f(XX-H[2]/ratio,YY-W[2]/ratio,0.0f);	// Bottom Left
glTexCoord2fv(m_coords[(int)id]->c10); glVertex3f(XX+H[2]/ratio,YY-W[2]/ratio,0.0f);	// Bottom Right
glTexCoord2fv(m_coords[(int)id]->c11); glVertex3f(XX+H[2]/ratio,YY+W[2]/ratio,0.0f);	// Top Right
glTexCoord2fv(m_coords[(int)id]->c01); glVertex3f(XX-H[2]/ratio,YY+W[2]/ratio,0.0f);	// Top Left
glEnd();
