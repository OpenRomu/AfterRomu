...
bouton::bouton():tps_vie(0.0f),enabled(true),visible(false),m_mousemove(false),m_mouseclick(false),m_release(0),m_tex(NULL),m_coords(NULL) ,m_font(NULL)
{
...
void bouton::SetFont(Font * in_font)
{
		m_font = in_font;
}
...
void bouton::SetText(char * in_text)
{
		strcpy(text,in_text);
...
void bouton::SetTexture(char * in_texture)
{
	...
	H=img.height();
	m_coords = new Coords;
	m_coords->c00[0] = 0; m_coords->c00[1] = 0;
	m_coords->c10[0] = 0+1; m_coords->c10[1] = 0;
	m_coords->c11[0] = 0+1, m_coords->c11[1] = 0+1;
	m_coords->c01[0] = 0; m_coords->c01[1] = 0+1;
...
glTexCoord2fv(m_coords->c00); glVertex3f(x-oofx,m_height-oofx-y,0.0f);	// Bottom Left
...
glTexCoord2fv(m_coords->c01); glVertex3f(x-oofx,m_height+oofx-y+h,0.0f);	// Top Left
glEnd();
