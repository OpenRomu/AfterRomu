...
Engine::Engine()
    ...
    , m_ycenter(0.0f)
    , m_playerfile(NULL)
    , g_bWasLobbyLaunched(FALSE)
    , g_dpnidLocalPlayer(0)
    , g_lNumberOfActivePlayers(0)
    , release_tir_car(true)
...
*/
    ...
    hr = CoCreateInstance(
        CLSID_DirectPlay8Address, NULL, CLSCTX_INPROC_SERVER, IID_IDirectPlay8Address, (LPVOID*)&m_pDeviceAddress);
    // if( FAILED(hr) )
    //  return DXTRACE_ERR( TEXT("CoCreateInstance"), hr );
    // Create a host address
    SAFE_RELEASE(m_pHostAddress);
    hr = CoCreateInstance(
        CLSID_DirectPlay8Address, NULL, CLSCTX_INPROC_SERVER, IID_IDirectPlay8Address, (LPVOID*)&m_pHostAddress);
    // if( FAILED(hr) )
    //   return DXTRACE_ERR( TEXT("CoCreateInstance"), hr );
 ...
 */
    ...
    for (int da = 0; da < lesbtn.size(); da++) {
        if (lesbtn[da]) {
            if (lesbtn[da]->visible && da != 40) // 11 c l'image de la map pas ici plus loin
                lesbtn[da]->frame(
                    m_input.xpos, m_input.ypos, m_width, m_height, m_input.left_button, config.isdebug, delta);
        }
        lesbtn[da]->visible = (
    ...
    if ((menu_state == 51)) { // join session  enum
        ...
        if ((lesbtn[14]->m_mouseclick)) { // quit
        ...
        if (m_input.keys[VK_RETURN] || lesbtn[13]->m_mouseclick) { // quit
        ...
        */
    ...
    } else if (menu_state == 110) { // join session  enum
        //	glColor3f(1.0f, 1.0f,0.0f);
        //	glDisable(GL_BLEND);
        //
        //	glColor4f(0.0f,0.3f,1.0f,0.5f);

        if (m_input.xpos > xd && lesbtn[15]->m_mouseclick) { // quit
        ...
        if (m_playerfile) {
            ...
            if (m_playerfile->player_id == 0) {
                ...
                char bufff[500];
                sprintf(bufff,
                    "Vous etes deja inscris*Saisissez votre pseudo*et votre mot de passe.*Sinon allez vous inscrire "
                    "sur*%s",
        ...
        */
        ...
        if ((lesbtn[38]->m_mouseclick)) { // quit
            mode_op = true;
        }
        if ((lesbtn[39]->m_mouseclick)) { // quit
            mode_op = false;
        }
        if ((lesbtn[47]->m_mouseclick)) { // quit
            menu_state = 2565;
        ...
        */
        ...
        if (lesbtn[44]->m_mouseclick) // rejoindre
        {
            ...
            hr = m_pHostAddress->AddComponent(
                DPNA_KEY_HOSTNAME, whostname, (wcslen(whostname) + 1) * sizeof(WCHAR), DPNA_DATATYPE_STRING);
        ...
        if (!(!config.isdebug && !lan_mode && strlen(MapList.List[curmapid].wad_md5) > 0
                && (strcmp(cur_md5, MapList.List[curmapid].wad_md5)
                    || strcmp(cur_md5_bsp, MapList.List[curmapid].bsp_md5)))) {
            if (Verify_Map(m_cur_map)) {
                load_map(m_cur_map);
...
*/
                    ...
                    if (m_input.xpos > xd && m_input.xpos < xd * 3.4 && pos_y_mouse > y1 - (i + 1) * h
                        ...
                        if (m_input.left_button && ((i - 1 + offset) < lists.count)) {
                            ...
                            hr = m_pHostAddress->AddComponent(DPNA_KEY_HOSTNAME, whostname,
                                (wcslen(whostname) + 1) * sizeof(WCHAR), DPNA_DATATYPE_STRING);
    ...
    if (menu_state == 351) {
        ...
        if (m_xmlsession.GetTournoisDetail(
                &lesekip, m_playerfile, tournoisv.at(curr_tournois).cle_noeud, tournoisv.at(curr_tournois).cle_tournois)
            == S_OK) {
            menu_state = 353;
        } else {
            menu_state = 351;
        ...
        if (menu_state == 9) {
            if (m_playerfile) {
                if (m_playerfile->player_id == 0) { // pas enregistré
            ...
            if (menu_state == 3) {
                ...
                /*if (menu_state==13)
                {
                 if (!m_input.left_button)
                ...
                else if (menu_state == 8) { // sortie de new player
                    ...
                    if (!m_input.left_button) {
                        if (strlen(m_playerfile->player_name) == 0) {
                            ...
                            if (m_playerfile) {
                                ...
                                m_playerfile->player_id = m_xmlsession.NewPlayer(m_playerfile->player_name,
                                    m_playerfile->playermail, m_playerfile->player_pwd, error_message);
                                if (m_playerfile->player_id != 0) {
                                    m_playerfile->save();
                ...
                } else if (menu_state == 150) { // sortie de new player
                    if (m_playerfile) {
                        if (!lan_mode)
                ...
                } else if (menu_state == 151) { // sortie de new player
                    ...
                    */
                    ...
                    if (m_input.keys[VK_RETURN] || lesbtn[9]->m_mouseclick || lesbtn[40]->m_mouseclick) { // Play
                    ...
                    if (curmapid + 1 < MapList.count) {
                        ...
                        if (lesbtn[11]->m_mouseclick) { // Play
...
//				SNIPER_ON=false;
                    ...
                    */
                    ...
                    if (lesbtn[8]->m_mouseclick) { // Play

                        if (m_dwMaxPlayers > 0)
                            m_dwMaxPlayers = m_dwMaxPlayers - 2;
                    }

                } else if (menu_state == 121) { // sortie de new player
                    ...
                    if (menu_state == 50) { // sortie de new player
            ...
            */
                ...
                else if (menu_state == 64) { // client en attente de deconnection
                    //	glColor3f(1.0f, 1.0f,0.0f);
                    //	glDisable(GL_BLEND);
                    //
                ...
                else if (menu_state == 63) { // attente fin click
                    if (Verify_Map(m_cur_map)) {
                        menu_state = 264;
                ...
                } else if (menu_state == 2264 && downloading == INET_IDLE) { // donwload map
                    // Download_Map (m_cur_host,m_cur_map);
                    sprintf(distant_file, "romustrike/map/%s.exe", m_cur_map);
                    sprintf(local_file, "%s.exe", m_cur_map);
                    type_download = 0;
                    ...
                    menu_state = 264;
                } else if (menu_state == 62) { // attente fin click
                    char b[500];
                ...
                } else if ((menu_state == 264) && (downloading == INET_IDLE)) { // load e la ma
...
void Engine::download_task()
{
    if (downloading != INET_IDLE) {
        ...
        if (downloading == INET_CONNECT) {
            ...
            hOpen = WinHttpOpen(
                L"InetURL/1.0", WINHTTP_ACCESS_TYPE_DEFAULT_PROXY, WINHTTP_NO_PROXY_NAME, WINHTTP_NO_PROXY_BYPASS, 0);
                ...
                if (!hConnect) {
                    ...
                    if (!hReq) {
                        ...
                        if (!WinHttpSendRequest(
                                hReq, WINHTTP_NO_ADDITIONAL_HEADERS, 0, WINHTTP_NO_REQUEST_DATA, 0, 0, 0)) {
    ...
    if (config.isdebug || mode_op) {
        if ((m_input.keys[VK_F8]) && !(relkeys[VK_F8])) {
            if (lejoueur[VRAI]->is_car) //
            {
                ...
                nouveau->SetPos(
                    lejoueur[VRAI]->pos + vec3_t(0.0f, 0.0f, 30.0f), lejoueur[VRAI]->voiture->Get_Global_velocity());
                ...
                lejoueur[VRAI]->voiture->SetPos(
                    lejoueur[VRAI]->pos - vec3_t(0.0f, 0.0f, 10.0f), vec3_t(0.0f, 0.0f, 0.0f));
...
if((m_input.keys['X']) && !(relkeys['X'])) {
    ...
    if (!(lejoueur[VRAI]->tir) && (lejoueur[VRAI]->occupe == false)
        && ((m_input.is_wheeling && m_input.wheel > 0)
            || (m_input.keys[config.keys[10]] && !relkeys[config.keys[10]]))) {
        if (!SCOPE_ETA) {
            Change_arme(true);
        } else {
    ...
    if (!(lejoueur[VRAI]->tir) && (lejoueur[VRAI]->occupe == false)
        && ((m_input.is_wheeling && m_input.wheel <= 0)
            || (m_input.keys[config.keys[7]] && !relkeys[config.keys[7]]))) {
        if (!SCOPE_ETA) {
            Change_arme(false);
        } else {
            lejoueur[VRAI]->arme.cur_scope -= 1.0f;
...
void Engine::migratehost()
{
    ...
    // menu_state=1111;
    if (m_playerfile && !lan_mode)
        m_serverid = m_xmlsession.DevenirServer(m_playerfile, m_cur_map, (CFT_ON | (TEAM_ON << 1) | (SNIPER_ON << 2)),
            m_dwMaxPlayers, cle_tournois, round, cur_md5, lesinput_box[6]->text.text,
            strlen(lesinput_box[5]->text.text) > 0, les_armes_autorisees); //
    // bConnectSuccess =true;
    m_bHostPlayer = TRUE;
    menu_state = 51;
    if (m_chat)
...
void Engine::hit_Romu(void) { m_panel->hit_Romu(); }
...
void Engine::HandleEvent(void) { mp3->HandleEvent(); }
                    ...
                    if (TOURNOIS_ON == 1 && (remain < 0)) {
                        ...
                        if (g_lNumberOfActivePlayers > 1) {
                            ...
                            for (int ij = 1; ij < g_lNumberOfActivePlayers; ij++) {
                                ...
                                m_pDP->SendTo(
                                    DPNID_ALL_PLAYERS_GROUP, &bufferDesc, 1, 0, NULL, &hAsync, DPNSEND_GUARANTEED);
                            }
    ...
    if (menu_mode) {
        ...
        if (lejoueur[VRAI]->mort == false) {
            ...
            if (lejoueur[VRAI]->mort == true) {
                ...
                if (world_is_loaded && (respawn_time <= 0)) { // on  relache lasouris respawn
            ...
            gluLookAt(
                dst_cam[0], dst_cam[1], dst_cam[2], vue_cam[0], vue_cam[1], vue_cam[2], 0, 0, 1); // orient ,pos,norm
                ...
                || !strcmp(lejoueur[VRAI]->arme.name, "fumigene") || !strcmp(lejoueur[VRAI]->arme.name, "plasma")) //
            {
                ...
                if (world_is_loaded && !strcmp(lejoueur[VRAI]->arme.name, "c4")) {
                    ...
                    if (!strcmp(lejoueur[VRAI]->arme.name, "grenade")) {
                        grenades->SystemNew(
                            new pGrenSystemSimple_t, lejoueur[VRAI]->pos, (flash2), 1, g_dpnidLocalPlayer);
                        ...
                        if (!strcmp(lejoueur[VRAI]->arme.name, "fumigene")) {
                            grenades->SystemNew(
                                new pGrenSystemSmoke_t, lejoueur[VRAI]->pos, (flash2), 1, g_dpnidLocalPlayer);
                            ok = true;
                ...
                if (exterieur) {
                    ...
                    if (!sniper) {
                        parts2->SystemNew(
                            new pSystemBurst_t, lejoueur[VRAI]->vue.StudioCalcAttachments(true), 1); // particle canon
                    }
            ...
            lesconsoles[da]->visible
                = menu_mode
                      && (((da == 0 || da == 2) && menu_state == 1100) //|| da==1
                          ...
                          || ((da == 12)
                              && (menu_state == 2052 || menu_state == 51 || menu_state == 64 || menu_state == 63
                                  || (menu_state == 351 && tournoisv.size() == 0))))
                  || ((da == 13) && (m_tchat > 0.0f));
        ...
        while (pDPHostEnum != &m_DPHostEnumHead) {
            ...
            if (pDPHostEnum->bValid) {
                ...
                if (m_input.xpos > xd && m_input.xpos < xd * 3 && m_input.ypos > yd * 3 - numb * 20 - 100
                    && m_input.ypos < yd * 3 - (numb - 1) * 20 - 100) { // quit
                    glColor4f(0.0f, 0.3f, 1.0f, 0.5f);
...
void Engine::Joueur_Creation(char* playername, DPNID ID, long cle_joueur)
{
    ...
    if (g_dpnidLocalPlayer == ID) { // c moi
    ...
    if (ID == g_dpnidLocalPlayer) {
        ...
        if ((m_bHostPlayer == TRUE)) {
            ...
            if (config.isdebug == false && !lan_mode) {
                m_serverid = m_xmlsession.DevenirServer(m_playerfile, m_cur_map,
                    (CFT_ON | (TEAM_ON << 1) | (SNIPER_ON << 2)), m_dwMaxPlayers, cle_tournois, round, cur_md5,
                    lesinput_box[6]->text.text, PASS_ON, les_armes_autorisees); //
                idpartie = m_xmlsession.JoinServer(m_playerfile, m_serverid);
            }
            menu_state = 51;
        } else {
...
void Engine::lockequipe() { EnterCriticalSection(&m_csTeam); }
void Engine::unlockequipe() { LeaveCriticalSection(&m_csTeam); }
void 
...
void Engine::load3d()
{
    ...
    */
    ...
    T_armes liste_armes[max_arme]=
    {
                    {"deagle",		1,	"_onehanded",	0,	10,	10,	0,	-1.0f,
    -1.0f,	1.0f,true},
                    {"mp5",			1,	"_mp5",			0,	20,	10,
    3,	30.0f,	10.0f,	2.0f,true},
                    {"smokegrenade",	1,	"_hegrenade",	0,	5,	0,	26,
    -1.0f,	8.0f,	1.5f,true},
                    {"silencieux",		1,	"_onehanded",	0,	10,	10,	2,	-1.0f,
    -1.0f,	1.5f,false},
                    {"xm1014",		1,	"_m249",		0,	5,	40,	8,	-1.0f,
    -1.0f,	1.0f,false},
                    {"mac10",		1,	"_onehanded",	0,	20,	5,
    1,	40.0f,	10.0f,	2.5f,true},
                    {"hegrenade",	1,	"_hegrenade",	0,	5,	0,	26,
    -1.0f,	8.0f,	1.5f,false},
                    {"m249",		1,	"_m249",		0,	40,	10,
    9,	30.0f,	10.0f,	2.0f,false},
                    {"plasma",		1,	"_m249",		0,	3,	0,
    28,	22.0f,	22.0f,	3.0f,true},
                    {"ak47",		1,	"_mp5",			0,	15,	15,
    4,	30.0f,	5.0f,	1.5f,true},
                    {"c4",			1,	"_c4",			0,	5,	0,	38,
    -1.0f,	8.0f,	1.5f,true},
                    {"famas",		1,	"_carbine",		0,	30,	20,
    39,	22.0f,	8.0f,	1.5f,true},
                    {"sg552",		1,	"_rifle",		10,	20,	20,
    6,	30.0f,	10.0f,	1.5f,true},
                    {"m4a1",		1,	"_rifle",		10,	20,	20,
    5,	30.0f,	5.0f,	1.5f,true},
                    {"awp",			1,	"_rifle",		20,	10,	100,7,	-1.0f,
    -1.0f,	1.0f,true},
                    {"ump45",		1,	"_carbine",		0,	30,	20,
    40,	30.0f,	8.0f,	2.0f,false},
                    {"m4a2",		1,	"_rifle",		10,	20,	20,
    5,	30.0f,	5.0f,	1.0f,false},
                    {"awp1",			1,	"_rifle",		20,	10,	100,7,	-1.0f,
    -1.0f,	1.0f,false},
                    {"awp2",			1,	"_rifle",		20,	10,	100,5,	-1.0f,
    -1.0f,	1.0f,false},
                    {"awp3",			1,	"_rifle",		20,	10,	100,7,	-1.0f,
    -1.0f,	1.0f,false}
    ...
    if (ret->pos[0] != 0.0f) {
        ...
        for (int j = 0; j < g_lNumberOfActivePlayers; j++) {
            if (lejoueur[j]->ID == idplayer && j != VRAI) {
                ...
                lapos.y = lejoueur[j]->pos[1];
                lapos.z = lejoueur[j]->pos[2];
                if (lejoueur[j]->id_arme_recu == 1 || lejoueur[j]->id_arme_recu == 4 || lejoueur[j]->id_arme_recu == 6
                    || lejoueur[j]->id_arme_recu == 7 || lejoueur[j]->id_arme_recu == 8) {
...
HRESULT Engine::Confirme_Tue(DPNID dpnidPlayer, float dans_la_tete)
{
    ...
    if (g_lNumberOfActivePlayers > 0) {
        ...
        */
        ...
        DPNHANDLE hAsync;
        m_pDP->SendTo(
            DPNID_ALL_PLAYERS_GROUP, &bufferDesc, 1, 0, NULL, &hAsync, DPNSEND_GUARANTEED | DPNSEND_PRIORITY_HIGH);
    }
    return S_OK;
}
    ...
    for (pass = 0; pass < 2; pass++) {
        for (ent_index = 0; ent_index < bsp->num_entities; ent_index++) {
            ...
            if (value[0]) {
                ...
                value = bsp->ValueForKey(&bsp->Lesentities[ent_index], "origin");
                if (value[0]) {
                    sscanf(value, "%f %f %f", &enttmp[0].origin[0], &enttmp[0].origin[1], &enttmp[0].origin[2]);
                }
        ...
        if (CFT_ON) {
            ...
            for (int j = 0; j < g_lNumberOfActivePlayers; j++) {
                ...
                if ((lejoueur[j]->etat == true) && (lejoueur[j]->ID == idplayer)) {
                    ...
                    if (ret->type_du_message == 0) {      // c la capture
                        if (lejoueur[j]->QuelTeam == 0) { // c le gign il a attrape flagTR
                            FlagTR.ID = idplayer;
                            FlagTR.eta_attrape();
...
HRESULT Engine::CFT_HOST_recoit_recapitulatif(DPNID idplayer, GAMEMSG_CFT_TOTALE* ret)
{
    ...
    if (FlagTR.ID != ret->Id_Flag_Tr) { // c la capture
    ...
    if (FlagCS.ID != ret->Id_Flag_Cs) { // c la capture
