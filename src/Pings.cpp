...
void
Pings::print(int m_width,int m_height,vector<Aplayer*>* joueur_list,int nb_gign,int nb_terro,bool CFT_ON,bool TEAM_ON,int CFT_eta,long CFT_count,long cft_timing,int FRIENDLY,bool SNIPER_ON)
{
	...
	int offy=100;
			...
			m_tex[0]->bind();
	 		id=0;
				...
				glTexCoord2fv(m_coords[(int)id]->c00); glVertex3f(50,HH/5,0.0f);	// Bottom Left
				glTexCoord2fv(m_coords[(int)id]->c10); glVertex3f(WW-50,HH/5,0.0f);	// Bottom Right
				glTexCoord2fv(m_coords[(int)id]->c11); glVertex3f(WW-50, HH/1.1,0.0f);	// Top Right
				glTexCoord2fv(m_coords[(int)id]->c01); glVertex3f(50,HH/1.1,0.0f);	// Top Left
				glEnd();
