...
public:
    ...
    vec3_t(const float* _v) { v[0] = _v[0], v[1] = _v[1], v[2] = _v[2]; }
    vec3_t(const vec3_t& _v) { v[0] = _v.v[0], v[1] = _v.v[1], v[2] = _v.v[2]; }
    ...
    //////////////////////////////////////////////////
    vec3_t operator+(const vec3_t& _v) const { return vec3_t(v[0] + _v.v[0], v[1] + _v.v[1], v[2] + _v.v[2]); }
    vec3_t operator-(const vec3_t& _v) const { return vec3_t(v[0] - _v.v[0], v[1] - _v.v[1], v[2] - _v.v[2]); }
    vec3_t operator*(const vec3_t& _v) const { return vec3_t(v[0] * _v.v[0], v[1] * _v.v[1], v[2] * _v.v[2]); }
    vec3_t operator*(float d) const { return vec3_t(v[0] * d, v[1] * d, v[2] * d); }
    vec3_t operator/(const vec3_t& _v) const { return vec3_t(v[0] / _v.v[0], v[1] / _v.v[1], v[2] / _v.v[2]); }
    ...
    float lenf() const { return (float)sqrtf((v[0] * v[0]) + (v[1] * v[1]) + (v[2] * v[2])); }
    ...
    {
        return vec3_t((v[1] * _v.v[2]) - (v[2] * _v.v[1]), (v[2] * _v.v[0]) - (v[0] * _v.v[2]),
            (v[0] * _v.v[1]) - (v[1] * _v.v[0]));
    }

    float dot(const vec3_t& _v) const { return v[0] * _v.v[0] + v[1] * _v.v[1] + v[2] * _v.v[2]; }
...
public:
    ...
    void perpendicular(vertex_t& a, vertex_t& b, vec3_t& normal) { *this = plane_t(a, b, b + normal); }

    plane_t perpendicular(const vertex_t& a, const vertex_t& b) const { return plane_t(a, b, b + n); }

    float dist_to_point(const vec3_t& pt) const { return n.dot(pt) + d; }

    bool intersect(const vec3_t& start, const vec3_t& dir, const vec3_t& pt, vec3_t& ret) const
    {
    ...
    bool intersect2(const vec3_t& start, const vec3_t& dir, const vec3_t& pt, vec3_t& ret) const
    {
        float denom = n.dot(dir);
