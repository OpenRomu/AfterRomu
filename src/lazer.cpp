...
bool pGrenSystemLazer_t::Frame(float& frametime, vec3_t& grav, vector<vec3_t>* lespos, vector<DPNID>* amoi,
    vector<vec3_t>* lesposjoueur, DPNID mon_id, vector<DPNID>* les_idjoueur, vector<Aplayer*>* lesjoueurs,
...
void pGrenSystemLazer_t::Die(void) { spawnTime = -1.0f; }
// vertex arrays?
...
void pGrenSystemLazer_t::Spawn(DPNID a_moi)
{
    ...
    if (fn[0] == 1 || fn[1] == 1 || fn[2] == 1) {
        if ((major == 0 && no[0] > 0) || major == 1) { // right.Set(0, 0, -1);
            right[1] = 0;
            right[2] = 1;
            right[0] = 0;
        }

        else if (major == 0) { // right.Set(0, 0, 1);
            right[1] = 0;
            right[2] = -1;
            right[0] = 0;
        } else {
    ...
    for (int i = 0; i < 1; i++) {
        ...
        n->life = ticker_t::PassedI() + (uint)(random_t::RandomRange(100000.0f, 100000.0f));
        n->size = 10.0f;
        n->vel = dirspawn; //
        n->pos = spawn - no;
        n->pold = spawn;
