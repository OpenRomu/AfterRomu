    ...
    , scroll(0)
{
    ...
    sprintf(Message, "%s",
        TEXT("********************************************************Bienvenue sur romustrike ****jeu multijoueur "
             "gratuit*******SVP*Entrez, sans crainte, une adresse email valide, elle ne sera pas utilise pour du "
             "spawn, cela vous permettra de recevoir un mail contenant vos "
             "identifiants.*******************************************************************"));
}
...
void Romu::init(void) { duree = 0; }
void Romu::show(void) { state = 0; }
void Romu::hide(void) { state = 1; }
void Romu::tchat_scroll(char* txt)
{
    sprintf(Message_Tchat,
        "                                                  %s                                                   \0",
        txt);
...
void Romu::print(int x, int y, int width, int height, float fps, int engine_state)
{
  ...
  */
    ...
    if (position > 0) {
        ...
        Coords* z = m_coords[0];
        glTexCoord2fv(z->c00);
        glVertex2s(x, y + (height - position));
        glTexCoord2fv(z->c10);
        glVertex2s(x + width, y + (height - position));
        glTexCoord2fv(z->c11);
        glVertex2s(x + width, y + height + (height - position));
        glTexCoord2fv(z->c01);
