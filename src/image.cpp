...
Image::Image()
: m_data(0), m_width(0), m_height(0), m_bpp(0), m_alpha(false)
{
}
...
bool
Image::load_bmp_true_color(FILE* f)
{
    int bytes = m_bpp>>3;
    unsigned int size = m_width*m_height*bytes;
    m_data = new unsigned char[size];
    if(!m_data) throw out_of_memory();
...
bool
Image::load_bmp_8bpp(FILE* f, unsigned int clr_used)
{
    bool r = false;
...
#ifdef _JPEG_SUPPORT
bool
Image::load_jpg(const char* filename)
{
    FILE* f = fopen(filename, "rb");
    if(!f) {
        return false;
