...
CPhysEnv::CPhysEnv():m_CurrentSys(NULL),m_TargetSys(NULL)
{
	frein_a_main=false;
...
my_world->drawBoxEx  (m_CurrentSys[7].pos);
	...
	vec3_t AxeX=((m_CurrentSys[4].pos+m_CurrentSys[7].pos)*0.5f)-((m_CurrentSys[5].pos+m_CurrentSys[6].pos)*0.5f);
	vec3_t AxeY=((m_CurrentSys[5].pos+m_CurrentSys[4].pos)*0.5f)-((m_CurrentSys[6].pos+m_CurrentSys[7].pos)*0.5f);
	vec3_t AxeZ=((m_CurrentSys[5].pos+m_CurrentSys[4].pos)*0.5f)-((m_CurrentSys[0].pos+m_CurrentSys[1].pos)*0.5f);
	vec3_t old_axe=AxeG;
	AxeG=(m_CurrentSys[6].pos+m_CurrentSys[7].pos+m_CurrentSys[4].pos+m_CurrentSys[5].pos)*0.25f;
	AxeDevant=((m_CurrentSys[5].pos+m_CurrentSys[4].pos)*0.5f)-((m_CurrentSys[6].pos+m_CurrentSys[7].pos)*0.5f);

	old_axe=AxeG-old_axe;
	
	GlobalVelo=old_axe.len();
	AxeHaut=((m_CurrentSys[5].pos+m_CurrentSys[4].pos)*0.5f)-((m_CurrentSys[0].pos+m_CurrentSys[1].pos)*0.5f);
...
//	 / NOW DO ALL THE SPRINGS
	...
	for (loop = 0; loop < m_SpringCnt; loop++)
	{
		...
		deltaP=p1->pos-p2->pos;	// Vector distance 
	 	dist = deltaP.len(); /// /!\ arrondi a la con a changer					// Magnitude of deltaP
...
inline double 
CPhysEnv::VectorLength(const vec3_t& v) const
{
	int		i;
	double	length;
...
mata[15]=1.0f; 
					...
					glLoadIdentity();
			 		 ...
			 		 glGetFloatv(GL_MODELVIEW_MATRIX, Matrix); //get current matrix
		glMultMatrixf(mat);	glTranslatef(10.0f,0.0,10.0f);
...
{
	vec3_t AxeX=((m_CurrentSys[5].pos+m_CurrentSys[4].pos)*0.5f)-((m_CurrentSys[6].pos+m_CurrentSys[7].pos)*0.5f);
	AxeX.normalize ();
	frein_a_main=fr;
        ...
        else
        {
            ...
            if(collisionState == COLLIDING)
            {
                ...
                do
                {
                    ...
                    Counter++;
                } while((CheckForCollisions(m_CurrentSys,m_TargetSys) ==
