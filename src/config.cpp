...
config_t::config_t(const char *filename)
    : xres(640), yres(480), msens(120), bpp(16), hz(60), vsync_off(false), fullscreen(false), isdebug(false), GAMMA(5),
      music_on(true), viseur(10)
{
    ...
    try
    {
        ...
        if (f)
        {
            ...
            sprintf(tmp,
                    "[debug]\n%s\n[gamma]\n%d\n[display]\n%d,%d,%d,%d,%s,%s\n[keys]\n%s\n[server_xml]\n%s\n[server_"
                    "tchat]\n%s\n[mouse]\n%d\n[music_on]\n%s\n[viseur]\n%d\n",
                    kdebug, GAMMA, xres, yres, bpp, hz, kvsync, kfull, touche, server_xml, server_tchat, msens,
                    kmusic_on, viseur);
