...
#define MAX_KEYS (256)
    class InputStatus
    {
      public:
        InputStatus()
            : left_button(false), right_button(false), xpos(0), ypos(0), wheel(0), is_wheeling(false),
              middle_button(false)
        {
  ...
  public:
    GLWindow()
        : m_width(0), m_height(0), m_bpp(0), m_fullscreen(0), m_hinst(0), m_hwnd(0), m_hdc(0), m_hrc(0), m_active(false)
    {
    }
    ~GLWindow()
    {
