...
struct pGrenadeManager_t
{
   ...
   pGrenSystem_t *GetSystems(void) {return systems;}

   void SetGravity(vec3_t &g) {gravity=g;}
   void SetId(int id, uint t) {if (id < pgNumTypes) ids[id] = t;}
   void SetWorld(world_t * world) {myworld=world; }
   void SetEXPLODE(pParticleManager_t* parts) {engineparts = parts;}
  
   void SystemNew(pGrenSystem_t *sys, vec3_t &pos, vec3_t &dir, bool die = 0,DPNID id_lanceur=0);
   void SystemDelete(pGrenSystem_t *sys);
   void Frame(float &frametime,vector<vec3_t >* lespos,vector<DPNID>*  amoi,vector<vec3_t>* lesposjoueur,DPNID mon_id,vector<DPNID>* lesid_joueur,vector<Aplayer *>* lesjoueurs,vector<CPhysEnv* > lescars );
   void Render(void);
