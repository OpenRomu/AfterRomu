...
public:
    ...
    // Engine();
    ~Engine();
    void SetpGuidApp(GUID* pGuidApp) { m_guidApp = *pGuidApp; }
    void Settcpguid(GUID* tcpguid) { ptcpGuid = *tcpguid; };
    void SetpDP(IDirectPlay8Peer* pDP) { m_pDP = pDP; };
    void SetpLB(IDirectPlay8LobbiedApplication* pLobbiedApp) { m_pLobbiedApp = pLobbiedApp; };
    void Sethinstance(HINSTANCE hinstance) { m_hInst = hinstance; };

    void Setlebuild(IGraphBuilder* lebuild) { m_pgraph = lebuild; };
    void Setpfn(const PFNDPNMESSAGEHANDLER pfn) { dphdl = pfn; };

    float float_to_shortint(float f) { return (f); };

    float shortint_to_float(float f) { return (f); };
