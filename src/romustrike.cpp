...
int getAdresseMac(int numeroInterface, char *adrMAC)
{
    ...
    if (numeroInterface <= lenum.length)
    {
        ...
        if (Netbios(&Ncb) == 0)
        {
            sprintf(adrMAC, "%02x%02x%02x%02x%02x%02x", Adapter.adapt.adapter_address[0],
...
HRESULT init_dplay()
{
    ...
    hr = CoCreateInstance(CLSID_DirectPlay8LobbiedApplication, NULL, CLSCTX_INPROC_SERVER,
                          IID_IDirectPlay8LobbiedApplication, (LPVOID *)&g_pLobbiedApp);

    if (FAILED(hr = CoCreateInstance(CLSID_FilterGraph, NULL, CLSCTX_INPROC_SERVER, IID_IGraphBuilder,
...
HRESULT WINAPI DirectPlayMessageHandler(PVOID pvUserContext, DWORD dwMessageId, PVOID pMsgBuffer)
{
    ...
    case DPN_MSGID_CREATE_PLAYER: //
    {
        ...
        if (hr == DPNERR_BUFFERTOOSMALL)
        {
            ...
            if (SUCCEEDED(hr))
            {
                ...
                DXUtil_ConvertWideStringToGeneric(pPlayerInfo->strPlayerName, pdpPlayerInfo->pwszName, MAX_PLAYER_NAME);

                if (pdpPlayerInfo->dwPlayerFlags & DPNPLAYER_LOCAL)
                    g_e->g_dpnidLocalPlayer = pCreatePlayerMsg->dpnidPlayer;

                g_e->lockequipe();
                g_e->Joueur_Creation(pPlayerInfo->strPlayerName, pCreatePlayerMsg->dpnidPlayer,
                                     pPlayerInfo->CLE_JOUEUR);
    ...
    case DPN_MSGID_RECEIVE: {
        ...
        {
            // g_e->colle_position(pPlayerInfo->dpnidPlayer,pMsg->pos,pMsg->rot,pMsg->ang_dos,pMsg->fps,pMsg->id_model
            // ,pMsg->id_arme ,pMsg->dep,pMsg->act,pMsg->sens);
    ...
    return g_e->MessageHandler(pvUserContext, dwMessageId, pMsgBuffer);
