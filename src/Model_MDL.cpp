...
Model_MDL::Model_MDL()
    : droitier(true), cur_scope(0), optimize(false), munition(0), le_fps_ori_seq2(0.0f), le_fps_ori_seq1(0.0f),
      m_frame(0), fps_arme(0), fps2(0), seq2(1), seq1(1), scope(0), multi(0), id_son(0), nb_boite(0), fps_weapon(0.0f),
      ang_dos(0.0f), m_pstudiohdr(NULL)
{
    m_sequence = 0;
...
int Model_MDL::calculeboite(vec3_t src, vec3_t normal)
{
    ...
    for (i = 0; i < m_pstudiohdr->numhitboxes; i++)
    {
        ...
        for (int j = 0; j < 8; j++)
        {
            ...
            for (int k = 0; k < 3; k++)
            {
                if (lemin[k] > poboite[i][j][k])
                {
                    lemin[k] = poboite[i][j][k];
                }
                if (lemax[k] < poboite[i][j][k])
                {
                    lemax[k] = poboite[i][j][k];
                }
            }
        }
...
float Model_MDL::intersect_sphere(const vec3_t &r,  // point E
...
void Model_MDL::SetupModel(int bodypart)
{
    ...
    mstudiobodyparts_t *pbodypart =
        (mstudiobodyparts_t *)((byte *)m_pstudiohdr + m_pstudiohdr->bodypartindex) + bodypart;
...
void Model_MDL::UploadTexture(mstudiotexture_t *ptexture, byte *data, byte *pal)
{
    ...
    gluBuild2DMipmaps(GL_TEXTURE_2D, GL_RGBA, outwidth, outheight, GL_RGBA, GL_UNSIGNED_BYTE, tex);
...
void Model_MDL::GetSequenceInfo(float *pflFrameRate, float *pflGroundSpeed)
{
    ...
    if (pseqdesc->numframes > 1)
    {
        ...
        *pflGroundSpeed = (float)sqrt(pseqdesc->linearmovement[0] * pseqdesc->linearmovement[0] +
...
float Model_MDL::SetBlending(int iBlender, float flValue)
{
    ...
    float setting = 255 * (flValue - pseqdesc->blendstart[iBlender]) /
