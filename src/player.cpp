...
Aplayer::Aplayer():cle_joueur(0),score(0),m_tex1(NULL) ,mort(false),m_tex2(NULL) ,m_tex3(NULL) ,m_tex4(NULL) ,parts(0) ,playersound (0)    ,saut(false),can_jump(true),tir(false),tir_grenade(false),ammo(0),grenade(0),smoke_grenade(0),
missile(2),Visible(true),killed(0),hauteur(0.0f),deja_en_cours(false),fire(false),fireg(false),is_car(false),id_texture_jeep(0),id_arme_recu(0),id_modele_recu(0),id_modele (0),id_weapon(0),nb_lazer(0),occupe(false)
{                  //FrameTime(0),
	team=false;
...
void 
Aplayer::init(CSoundManager*          g_pSoundManager)
{ 
 /*
...
void 
Aplayer::init_particle (world_t* world)
{
...
void 
Aplayer::Render_particle(float delta)
{
...
D3DVECTOR vv;
	...
	vv.y=0.0f;
			...
			{
				sequence_mort_deja_affecte=false; //**MULTI MORT 06/01/2005
				if (arme.munition==0 && strcmp(arme.name,"grenade") && strcmp(arme.name,"c4")  && strcmp(arme.name,"fumigene")  && strcmp(arme.name,"plasma") && occupe==false)
				{
					reload_arme();
				...
				{
					if  ( ( !strcmp(arme.name,"grenade") || !strcmp(arme.type,"fumigene") ) && arme.munition<=0 && occupe==false )
					{
		...
		else//joueur distant
		{
		 //fps1_recu=fps1_recu+1;
			...
			if ((modele.fps1!=fps1_recu) || (modele.fps2!=fps2_recu)|| (modele.seq1!=id_seq1_recu)||(modele.seq2!=id_seq2_recu))
			{
...
void Aplayer::affecte_sound(CSound* lessons)
{
	...
	playersound = new CSound( lessons->m_apDSBuffer, lessons->m_dwDSBufferSize, lessons->m_dwNumBuffers, lessons->m_pWaveFile );
...
void 
Aplayer::affecte_modele(Model_MDL *leshommes,BYTE id_mod,int max)
{
	...
	if ((id_modele==0) || (id_modele==3) || (id_modele==4) || (id_modele==7) || (id_modele==9) || (id_modele==11))
		QuelTeam=0;
	else
		QuelTeam=1;
...
void
Aplayer::affecte_arme(Model_MDL *lesarmes,BYTE id_arm,int max)
{
	if (id_arm>=max){id_arm=0;}
	/*if (max==100)
	{
