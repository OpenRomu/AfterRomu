...
float world_t::get_distance(const vec3_t& pos, const vec3_t& dir)
{
    ...
    if (idx == 0) {
        // on traverse !
        if (new_dir.len() > 0.1f) { // a voir avec un cutoff
            new_dir = new_dir / 2;
...

{
    ...
    */
    ...
    // Buffer Setup glDepthFunc(GL_LEQUAL);
    // // Type Of Depth Testing glDepthMask(1); glEnable(GL_DEPTH_TEST);
...
//   	normalizeVector(pNormal);
   ...
   if (dot(pNormal, normalizedVelocity) <= 1.0f) {
           ...
           if (!point_in_poly(pIPoint,f,eRadius )) { // CheckPointInTriangle(pIPoint,p1,p2,p3)if not in triangle
                           ...
                           distToEllipsoidIntersection = intersectRaySphere(polyIPoint, -normalizedVelocity,
source, 1.0f);
