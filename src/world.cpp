...
static bool
point_in_poly21(const vec3_t& p, vec3_t *pf)
{  double total = 0.0f;
...
static bool
point_in_poly(const vec3_t& p, const face_t* f,float epsilon)
{
    vec3_t v[32]; // BAD SHIT
	int i;
    for(i = 0; i < f->count; i++) {
...
static bool
point_in_poly2(const vec3_t& p, const face_t* f)
{  double total = 0.0f;
...
static vec3_t
closest_on_poly(const vec3_t& p, const face_t* f)
{
    vec3_t v[32];
    float d[32];
	int i;
    for(i = 0; i < f->count-1; i++) {
        int c = f->first + i;
        v[i] = closest_on_line(vec3_t(vt_array[c]),
                               vec3_t(vt_array[c+1]), 
                               p);
        vec3_t t = p-v[i];
        d[i] = t.len();
    ...
    i = f->count-1;
    v[i] = closest_on_line(vec3_t(vt_array[f->first + i]),
                           vec3_t(vt_array[f->first]),
                           p);
    vec3_t t = p-v[i];
    d[i] = t.len();
...
static bool
check_collision(const face_t* f, collision_data& coldat,float radius)
{
...
static void
check_collision_car(const face_t* f, collision_data& coldat,float radius)
{
    vec3_t r;
    vec3_t pt(vt_array[f->first]);
    vec3_t n = f->p.normal();
...
*/
 ...
 void
check_tir(const face_t* f, collision_data& coldat)
{
vec3_t r;
vec3_t pt(vt_array[f->first]);
vec3_t n = f->p.normal();
...
void
world_t::projection_setup(int width, int height)
{
...
void
world_t::frustum_setup(const Camera& c)
{
...
bool
world_t::frustum_cull(const short* mins, const short* maxs)
{
		for(int i = 0; i < 5; i++) {
        ...
        d += (*n>0 ? (*n*(float)*min) : (*n*(float)*max)); min++, max++; n++;
        d += (*n>0 ? (*n*(float)*min) : (*n*(float)*max)); min++, max++; n++;
        if(d>0) { clip_count++; return true; }
    }
    return false;
}

int
world_t::find_leaf(const vec3_t& coords)
{
	if(bsp)
	{
...
void
world_t::advance_frame(float delta_f)
{
	delta=delta+delta_f;
	if(delta>0.5f)
	{
...
void
world_t::render_face(const const face_t* f)
{
...
vec3_t
world_t::check_collisions(const vec3_t& src, const vec3_t& dir,int cutoff,bool fill_colide_face)
{
...
//p<<" end"<<endl;
 ...
 vec3_t
world_t::check_collisions_yeux(const vec3_t& src, const vec3_t& dir,int cutoff,bool fill_colide_face)
{
	float epsilon=0.05f;
...
void
world_t::check_collisions_bat(const vec3_t& src, vec3_t dir, collision_tir* out, float radius)
{
...
//////////////////////////////////////////////////////////////
 collision_tir
world_t::check_tirs(const vec3_t& src, const vec3_t& dir)
{
   	collision_tir tmp;
...
void
world_t::process_visible_faces_collide(const vec3_t& cam,const vec3_t& dest)
{
    rendered = 0;
...
c=EntVar[ii];
 ...
 //////////////////////////////////////////////////////////////
 collision_tir
world_t::check_tirs_rec(const vec3_t& src, const vec3_t& dir,int lecutoff)
{
   	collision_tir tmp;
	if (lecutoff>0 )
		{
...
return tmp;
 ...
 collision_tir
world_t::check_tirs_rec_old(const vec3_t& src, const vec3_t& dir,int lecutoff)
{
   	collision_tir tmp;
	if (lecutoff>0 )
		{
...
vec3_t
world_t::check_collisions2(const vec3_t& src, const vec3_t& dir,int cutoff)
{
	float epsi;
	epsi=0.001f;
...
void
world_t::process_visible_faces(const Camera& c)
{
...
void
world_t::process_visible_faces_col(const vec3_t& cam)
{
    rendered = 0;
...
void
world_t::process_visible_faces3(const vec3_t& cam)
{
    rendered = 0;
    // setup for frustum culling
    //frustum_setup(c);
	...
	/*rendered=0; 
	 ...
	 //for(int x = 0; x < bsp->num_entities; x++) {
		/////o << bsp->ValueForKey(&bsp->Lesentities[x],"classname") << endl; 
		 ...
		 //if ((strcmp(bsp->ValueForKey(&bsp->Lesentities[x],"classname"),"info_player_start")==0)||(strcmp(bsp->ValueForKey(&bsp->Lesentities[x],"classname"),"info_player_deathmatch")==0))
			vec3_t po[8];
			short bbmin[3];
			short bbmax[3];
			short bbmin2[3];
...
float
world_t::get_distance(const vec3_t& pos,const vec3_t& dir)
{
	vec3_t dst ;
	vec3_t new_dir ;
...
void
world_t::render_visible_faces(vec3_t &pos_light,bool is_light)
...
GLfloat position[4];
position[0]=pos_light[0]; position[1]=pos_light[1]; position[2]=pos_light[2];
position[3]=1.0f;  
...
void
world_t::load(const char* file,int gamma)
{
...
got_flag=false;
  ...
  if(!bsp) throw out_of_memory();
	
     // basic setup
    decompress();
    // setup...
    mark_faces = new bitset(bsp->face_count);
    if(!mark_faces) throw out_of_memory();
    ...
    strncpy( buff, ss.c_str(), 30 );
        ...
        for(c = 0; c < f->numedges; c++) {
            ...
            // compute extents
            for(int x = 0; x < 2; x++) {
                float d = v->point[0] * ti->vecs[x][0] +
                          v->point[1] * ti->vecs[x][1] +
                          v->point[2] * ti->vecs[x][2] +
                          ti->vecs[x][3];
...
face->lightofs=f->lightofs;
			...
			if (lsz<=1000) //SHOGUN WAYOFGUN XP
			{
				...
				for(c = 0; c < 1; c++) {
					...
					face->lightmaps[c] = 
						new Texture(r.data, r.width, r.height, GL_RGB,
...
void
world_t::ChangeGamma(unsigned char  *image, int size, float factor)
	{
...
bool
world_t::LeJoueur_est_il_visible(const Camera& c,const vec3_t& pos)
{
...
void
world_t::AffecteGammaSurUneFace(int gamma)
{
